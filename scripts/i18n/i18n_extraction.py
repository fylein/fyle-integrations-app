...
4. **Replace hardcoded user-facing strings with Transloco usage:**
   - **Angular 18 Binding Rules:**
     ...
     - For text content, use the transloco pipe:  
       {{ 'componentName.keyName' | transloco }}
...
def call_gemini(prompt, batch=None, batch_num=None):
    ...
    if batch is not None and batch_num is not None:
        ...
        with open('scripts/failed_batches.jsonl', 'a', encoding='utf-8') as fail_f:
...
def main():
    ...
    base_dir = f'scripts/i18n/{args.type}'
