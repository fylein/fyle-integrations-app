...
export class TranslocoHttpLoader implements TranslocoLoader {
  ...
  private deepMerge(target: Translation, source: Translation): Translation {
    ...
    for (const key in source) {
      if (source.hasOwnProperty(key)) {
        if (key in result && typeof result[key] === 'object' && result[key] !== null && !Array.isArray(result[key]) && typeof source[key] === 'object' &&  source[key] !== null && !Array.isArray(source[key])) {
  ...
  getTranslation(lang: string): Observable<Translation> {
    ...
    return forkJoin({
      ...
      this.deepMerge(brandSpecificContent, app)
    ));
  }
}
