...
export class XeroDashboardComponent implements OnInit, OnDestroy {
  ...
  accountingExportType: XeroTaskLogType[] = [
    XeroTaskLogType.FETCHING_EXPENSE,
    XeroTaskLogType.CREATING_BILL,
    XeroTaskLogType.CREATING_BANK_TRANSACTION,
  ];
        ...
        if (
          ...
          forkJoin([
            ...
            this.exportableAccountingExportIds = res.results
              ...
              .map((taskLog: { expense_group: any }) => taskLog.expense_group);
  ...
  private setupPage(): void {
    ...
    forkJoin([
      ...
      this.errors = DashboardModel.parseAPIResponseToGroupedError(responses[0]);
      this.reimbursableImportState = responses[4].workspace_general_settings?.reimbursable_expenses_object
        ? this.reimbursableExpenseImportStateMap[responses[4].expense_group_settings.reimbursable_expense_state]
        : null;
      this.cccImportState = responses[4].workspace_general_settings?.corporate_credit_card_expenses_object
      ...
      const queuedTasks: XeroTaskLog[] = responses[2].results.filter(
        (task: XeroTaskLog) => task.status === TaskLogState.ENQUEUED || task.status === TaskLogState.IN_PROGRESS,
      );
      this.failedExpenseGroupCount = responses[2].results.filter(
        (task: XeroTaskLog) => task.status === TaskLogState.FAILED || task.status === TaskLogState.FATAL,
      ).length;
