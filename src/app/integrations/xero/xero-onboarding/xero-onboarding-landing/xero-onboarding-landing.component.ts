...
export class XeroOnboardingLandingComponent implements OnInit, OnDestroy {
  ...
  connectXero() {
    ...
    const url = `${environment.xero_authorize_uri}?client_id=${environment.xero_oauth_client_id}&scope=${environment.xero_scope}&response_type=code&redirect_uri=${environment.xero_oauth_redirect_uri}&state=qbo_local_redirect`;
