...
export class TravelperkPaymentProfileSettingsComponent implements OnInit {
  ...
  constructPayloadAndSave(): void {
    ...
    const paymentProfileMappingPayload = this.travelperkPaymentProfileSettingService.createPaymentProfileSettingPayload(
      this.paymentProfileMappingForm,
    );
    this.travelperkService.postTravelperkPaymentProfileMapping(paymentProfileMappingPayload).subscribe(
      (travelperkPaymentProfileMappingResponse) => {
        this.isSaveInProgress = false;
        this.toastService.displayToastMessage(
          ToastSeverity.SUCCESS,
          this.translocoService.translate('travelperkPaymentProfileSettings.paymentProfileSettingsSuccess'),
        );
        this.trackingService.trackTimeSpent(
          TrackingApp.TRAVELPERK,
          Page.PAYMENT_PROFILE_SETTINGS_TRAVELPERK,
      ...
      () => {
        ...
        this.toastService.displayToastMessage(
          ToastSeverity.ERROR,
          this.translocoService.translate('travelperkPaymentProfileSettings.paymentProfileSettingsError'),
        );
      },
    );
  }
  ...
  getProfileMappings(limit: number) {
    this.travelperkService.getTravelperkPaymentProfileMapping(limit).subscribe(
      (travelperkPaymentProfileMappingResponse: TravelperkPaymentProfileSettingResponse) => {
        this.paymentProfileSettings = travelperkPaymentProfileMappingResponse;
        this.paymentProfileMappingForm = this.travelperkPaymentProfileSettingService.mapAPIResponseToFormGroup(
          this.paymentProfileSettings.results,
        );
