...
export class QboCloneSettingsComponent implements OnInit {
  ...
  save(): void {
    ...
    const cloneSettingPayload = this.qboCloneSettingsService.constructPayload(
    ...
    this.cloneSettingService.postCloneSettings(cloneSettingPayload).subscribe(
      (response) => {
        ...
        this.toastService.displayToastMessage(
          ...
          this.translocoService.translate('qboCloneSettings.clonedSettingsSuccess'),
        );
      ...
      () => {
        ...
        this.toastService.displayToastMessage(
          ToastSeverity.ERROR,
          this.translocoService.translate('qboCloneSettings.failedToCloneSettings'),
        );
      },
    );
  }
  ...
  isAutoCreateVendorsFieldVisible(): boolean {
    return (
      this.cloneSetting.employee_mappings.workspace_general_settings.employee_field_mapping ===
        EmployeeFieldMapping.VENDOR &&
      this.cloneSetting.employee_mappings.workspace_general_settings.auto_map_employees !== null &&
      this.cloneSetting.employee_mappings.workspace_general_settings.auto_map_employees !==
        AutoMapEmployeeOptions.EMPLOYEE_CODE
    );
  }

  isPaymentSyncFieldVisible(): boolean {
    return (
      this.cloneSetting.export_settings.workspace_general_settings.reimbursable_expenses_object ===
  ...
  isSingleCreditLineJEFieldVisible(): boolean {
    return (
      ...
      this.cloneSetting.export_settings.workspace_general_settings.corporate_credit_card_expenses_object ===
  ...
  isAutoCreateMerchantsAsVendorsFieldVisible(): boolean {
    return (
      !this.cloneSetting.import_settings.workspace_general_settings.import_vendors_as_merchants &&
      (this.cloneSetting.export_settings.workspace_general_settings.corporate_credit_card_expenses_object ===
        QBOCorporateCreditCardExpensesObject.CREDIT_CARD_PURCHASE ||
        this.cloneSetting.export_settings.workspace_general_settings.corporate_credit_card_expenses_object ===
          QBOCorporateCreditCardExpensesObject.DEBIT_CARD_EXPENSE ||
        this.cloneSetting.export_settings.workspace_general_settings.name_in_journal_entry ===
          NameInJournalEntry.MERCHANT)
    );
  }

  private setupOnboardingSteps(): void {
    const onboardingSteps = this.qboOnboardingService.getOnboardingSteps(
      'Clone settings',
  ...
  private setupCustomWatchers(): void {
    ...
    this.exportSettingService.creditCardExportTypeChange.subscribe(
  ...
  private updateCCCExpenseGroupingDateOptions(selectedValue: QBOCorporateCreditCardExpensesObject): void {
    if (
      ...
      this.cccExpenseGroupingDateOptions =
        this.qboExportSettingsService.getAdditionalCreditCardExpenseGroupingDateOptions();
      ...
      if (!isImportCategoriesEnabled) {
        ...
        this.importSettingForm.controls.importCategoryCode.setValue(
          this.importSettingsService.getImportCodeField(
            this.cloneSetting.import_settings.workspace_general_settings.import_code_fields,
            DefaultImportFields.ACCOUNT,
            this.cloneQboImportCodeFieldCodeConfig,
          ),
        );
      }
      if (isImportCategoriesEnabled) {
  ...
  private setupPage(): void {
    ...
    const destinationAttributes = [
      QBOField.BANK_ACCOUNT,
      QBOField.CREDIT_CARD_ACCOUNT,
      QBOField.ACCOUNTS_PAYABLE,
      QBOField.VENDOR,
      QBOField.TAX_CODE,
    ];
    ...
    forkJoin([
      ...
      ([
        ...
        this.qboImportSettingsService.getImportCodeFieldConfig().subscribe((cloneQboImportCodeFieldCodeConfig) => {
          ...
          // Employee Settings
          this.employeeSettingForm = QboEmployeeSettingsService.parseAPIResponseToFormGroup(
            cloneSetting.employee_mappings,
          );
          ...
          this.cccAccounts = destinationAttributes.CREDIT_CARD_ACCOUNT.map((option: DestinationAttribute) =>
            this.qboExportSettingsService.formatGeneralMappingPayload(option),
          );
          this.accountsPayables = destinationAttributes.ACCOUNTS_PAYABLE.map((option: DestinationAttribute) =>
            this.qboExportSettingsService.formatGeneralMappingPayload(option),
          );
          this.vendors = destinationAttributes.VENDOR.map((option: DestinationAttribute) =>
            this.qboExportSettingsService.formatGeneralMappingPayload(option),
          );
          ...
          this.reimbursableExportTypes = this.qboExportSettingsService.getReimbursableExportTypeOptions(
            cloneSetting.employee_mappings.workspace_general_settings.employee_field_mapping,
          );
          this.showNameInJournalOption =
            cloneSetting.export_settings.workspace_general_settings?.corporate_credit_card_expenses_object ===
            QBOCorporateCreditCardExpensesObject.JOURNAL_ENTRY
              ? true
              : false;
          ...
          if (cloneSetting.export_settings.workspace_general_settings.reimbursable_expenses_object) {
            this.exportSettingService.setupDynamicValidators(
              this.exportSettingForm,
              employeeMappingControl,
              exportModuleRule[0],
              cloneSetting.export_settings.workspace_general_settings.reimbursable_expenses_object,
            );
          }
          ...
          this.exportSettingService.setExportTypeValidatorsAndWatchers(
            exportModuleRule,
            this.exportSettingForm,
            this.employeeSettingForm.get('employeeMapping'),
          );
          ...
          this.isImportMerchantsAllowed =
            !cloneSetting.advanced_configurations.workspace_general_settings.auto_create_merchants_as_vendors;
          ...
          this.importSettingForm = this.qboImportSettingsService.mapAPIResponseToFormGroup(
          ...
          this.fyleFields.push({
            ...
            display_name: this.translocoService.translate('qboCloneSettings.createCustomField'),
            is_dependent: false,
          });
          ...
          if (
            this.cloneSetting.advanced_configurations.workspace_schedules?.additional_email_options &&
            this.cloneSetting.advanced_configurations.workspace_schedules?.additional_email_options.length > 0
          ) {
          ...
          this.advancedSettingForm = this.qboAdvancedSettingsService.mapAPIResponseToFormGroup(
            this.cloneSetting.advanced_configurations,
            false,
            this.adminEmails,
          ...
          this.defaultMemoOptions = AdvancedSettingsService.getMemoOptions(
            this.cloneSetting.export_settings,
            AppName.QBO,
          );
