...
export class QboExportSettingsComponent implements OnInit {
  ...
  previewImagePaths = [
    ...
    },
    {
      ...
      [QBOCorporateCreditCardExpensesObject.CREDIT_CARD_PURCHASE]:
        'assets/illustrations/qbo/CCC-credit-card-purchase.png',
  ...
  constructor(
    ...
    this.reimbursableExpenseGroupingDateOptions =
      this.qboExportSettingsService.getReimbursableExpenseGroupingDateOptions();
    ...
    this.showWarningMessageAboutAutoExpenseCreation =
      this.storageService.get('showWarningMessageAboutAutoExpenseCreationInQBO') ?? true;
  }
  ...
  isEmployeeMappingDisabled(): boolean {
    ...
    return exportType === QBOReimbursableExpensesObject.BILL || exportType === QBOReimbursableExpensesObject.CHECK;
  }
      ...
      concat(
        ...
        complete: () => {
          ...
          this.toastService.displayToastMessage(
            ...
            this.translocoService.translate('qboExportSettings.exportSettingsSuccess'),
            undefined,
            this.isOnboarding,
          );
        ...
        error: () => {
          ...
          this.toastService.displayToastMessage(
            ToastSeverity.ERROR,
            this.translocoService.translate('qboExportSettings.exportSettingsError'),
          );
        },
      });
    }
  ...
  private setLiveEntityExample(destinationAttributes: DestinationAttribute[]): void {
    this.liveEntityExample = {
      [FyleField.EMPLOYEE]: destinationAttributes.find(
        (attribute: DestinationAttribute) => attribute.attribute_type === FyleField.EMPLOYEE,
      )?.value,
      [FyleField.VENDOR]: destinationAttributes.find(
        (attribute: DestinationAttribute) => attribute.attribute_type === FyleField.VENDOR,
      )?.value,
    };
  }
  ...
  private replaceContentBasedOnConfiguration(
    ...
    exportType: 'reimbursable' | 'credit card',
  ): string {
    const translatedExportType = this.translocoService.translate(
      `qboExportSettings.${exportType.replace(' ', '')}ExpenseType`,
    );
    const configurationUpdate = this.translocoService.translate('qboExportSettings.configurationUpdateWarning', {
    ...
    const newConfiguration = this.translocoService.translate('qboExportSettings.newConfigurationWarning', {
      exportType: translatedExportType,
    });
  ...
  private constructWarningMessage(): string {
    let content: string = '';
    const existingReimbursableExportType = this.exportSettings.workspace_general_settings?.reimbursable_expenses_object
      ? this.exportSettings.workspace_general_settings.reimbursable_expenses_object
      : 'None';
    const existingCorporateCardExportType = this.exportSettings.workspace_general_settings
      ?.corporate_credit_card_expenses_object
      ? this.exportSettings.workspace_general_settings.corporate_credit_card_expenses_object
      : 'None';
    const updatedReimbursableExportType = this.exportSettingForm.get('reimbursableExportType')?.value
      ? this.exportSettingForm.get('reimbursableExportType')?.value
      : 'None';
    const updatedCorporateCardExportType = this.exportSettingForm.get('creditCardExportType')?.value
    ...
    if (this.isSingleItemizedJournalEntryAffected()) {
      if (updatedReimbursableExportType !== existingReimbursableExportType) {
        content = this.replaceContentBasedOnConfiguration(
          ...
          this.translocoService.translate('qboExportSettings.reimbursableExpenseType'),
        );
        ...
        content = this.replaceContentBasedOnConfiguration(
          ...
          this.translocoService.translate('qboExportSettings.creditCardExpenseType'),
        );
      }
    }
  ...
  private isExportSettingsUpdated(): boolean {
    return (
      this.exportSettings.workspace_general_settings?.reimbursable_expenses_object !== null ||
      this.exportSettings.workspace_general_settings?.corporate_credit_card_expenses_object !== null
    );
  }
  ...
  private isAdvancedSettingAffected(): boolean {
    if (
      this.isExportSettingsUpdated() &&
      (this.isSingleItemizedJournalEntryAffected() || this.isPaymentsSyncAffected())
    ) {
      return true;
    }
  ...
  private setupCustomWatchers(): void {
    ...
    this.qboExportSettingsService.creditCardExportTypeChange.subscribe(
      (selectedValue: QBOCorporateCreditCardExpensesObject) => {
        this.showNameInJournalOption =
          selectedValue === QBOCorporateCreditCardExpensesObject.JOURNAL_ENTRY ? true : false;
  ...
  private setupCustomDateOptionWatchers(): void {
    this.exportSettingForm.controls.reimbursableExportGroup?.valueChanges.subscribe((reimbursableExportGroup) => {
      this.reimbursableExpenseGroupingDateOptions = this.qboExportSettingsService.constructExportDateOptions(
        false,
  ...
  private handleOptionSearch(event: ExportSettingOptionSearch): void {
    ...
    if (
      event.destinationOptionKey === QboExportSettingDestinationOptionKey.EXPENSE_ACCOUNT ||
      event.destinationOptionKey === QboExportSettingDestinationOptionKey.BANK_ACCOUNT_AND_CREDIT_CARD_ACCOUNT
    ) {
  ...
  private getExistingOptions(
    destinationOptionKey: QboExportSettingDestinationOptionKey,
  ): DefaultDestinationAttribute[] {
  ...
  private handleExpenseAccountSearch(
    event: ExportSettingOptionSearch,
    existingOptions: DefaultDestinationAttribute[],
  ): void {
    forkJoin([
      ...
      this.updateOptions(
        event.destinationOptionKey as QboExportSettingDestinationOptionKey,
        expenseAccounts,
        existingOptions,
      );
    });
  ...
  private handleGeneralOptionSearch(
    event: ExportSettingOptionSearch,
    existingOptions: DefaultDestinationAttribute[],
  ): void {
    this.getPaginatedAttributes(
      event.destinationOptionKey as QboExportSettingDestinationOptionKey,
      event.searchTerm,
    ).subscribe((response) => {
      this.updateOptions(
  ...
  private getPaginatedAttributes(
    destinationOptionKey: QboExportSettingDestinationOptionKey,
    searchTerm: string,
  ): Observable<any> {
    return this.mappingService
      .getPaginatedDestinationAttributes(destinationOptionKey, searchTerm)
  ...
  private setUpdatedOptions(
    destinationOptionKey: QboExportSettingDestinationOptionKey,
    updatedOptions: DefaultDestinationAttribute[],
  ): void {
    switch (destinationOptionKey) {
      case QboExportSettingDestinationOptionKey.ACCOUNTS_PAYABLE:
  ...
  private addMissingOptions() {
    if (this.exportSettings.general_mappings) {
      ...
      this.helperService.addDefaultDestinationAttributeIfNotExists({
        options: this.bankAccounts,
        newOption: this.exportSettings.general_mappings.bank_account,
      });
      this.helperService.addDefaultDestinationAttributeIfNotExists({
        options: this.expenseAccounts,
        newOption: this.exportSettings.general_mappings.qbo_expense_account,
      });
      this.helperService.addDefaultDestinationAttributeIfNotExists({
        options: this.accountsPayables,
        newOption: this.exportSettings.general_mappings.accounts_payable,
      });

      this.helperService.addDefaultDestinationAttributeIfNotExists({
        options: this.cccAccounts,
        newOption: this.exportSettings.general_mappings.default_ccc_account,
      });
      this.helperService.addDefaultDestinationAttributeIfNotExists({
        options: this.expenseAccounts,
        newOption: this.exportSettings.general_mappings.default_debit_card_account,
      });
      this.helperService.addDefaultDestinationAttributeIfNotExists({
        options: this.vendors,
        newOption: this.exportSettings.general_mappings.default_ccc_vendor,
      });
    }
  }
  ...
  private getSettingsAndSetupForm(): void {
    ...
    forkJoin([
      ...
      ([
        ...
        this.bankAndLiabilityAccounts = bankAccounts.results.map((option) =>
          this.qboExportSettingsService.formatGeneralMappingPayload(option),
        );

        const bankAccountOptions = bankAccounts.results.filter((option) => option.detail?.account_type === 'Bank');
        this.bankAccounts = bankAccountOptions.map((option) =>
          this.qboExportSettingsService.formatGeneralMappingPayload(option),
        );

        this.cccAccounts = cccAccounts.results.map((option) =>
          this.qboExportSettingsService.formatGeneralMappingPayload(option),
        );
        this.accountsPayables = accountsPayables.results.map((option) =>
          this.qboExportSettingsService.formatGeneralMappingPayload(option),
        );
        this.vendors = vendors.results.map((option) =>
          this.qboExportSettingsService.formatGeneralMappingPayload(option),
        );
        ...
        this.exportSettingForm = this.qboExportSettingsService.mapAPIResponseToFormGroup(
          this.exportSettings,
        ...
        if (this.exportSettings.workspace_general_settings?.reimbursable_expenses_object) {
          this.qboExportSettingsService.setupDynamicValidators(
            this.exportSettingForm,
            employeeMappingControl,
            exportModuleRule[0],
            this.exportSettings.workspace_general_settings?.reimbursable_expenses_object,
          );
          this.helperService.setOrClearValidators(
            this.exportSettings.workspace_general_settings?.reimbursable_expenses_object,
            exportSettingValidatorRule.reimbursableExpense,
            this.exportSettingForm,
          );
        }
        ...
        if (this.exportSettings.workspace_general_settings?.corporate_credit_card_expenses_object) {
          ...
          this.helperService.setOrClearValidators(
            this.exportSettings.workspace_general_settings?.corporate_credit_card_expenses_object,
            exportSettingValidatorRule.creditCardExpense,
            this.exportSettingForm,
          );
        }
