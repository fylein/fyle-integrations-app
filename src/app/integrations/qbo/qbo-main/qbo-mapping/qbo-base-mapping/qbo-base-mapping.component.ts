...
export class QboBaseMappingComponent implements OnInit {
    ...
    this.mappingService.triggerAutoMapEmployees().subscribe(
      () => {
        ...
        this.toastService.displayToastMessage(
          ToastSeverity.INFO,
          this.translocoService.translate('qboBaseMapping.autoMappingInProgress'),
        );
      },
      () => {
        this.isLoading = false;
        this.toastService.displayToastMessage(
          ToastSeverity.ERROR,
          this.translocoService.translate('qboBaseMapping.autoMappingError'),
        );
      },
    );
  }
  ...
  private getDestinationField(
    workspaceGeneralSetting: QBOWorkspaceGeneralSetting,
  ...
  private setupPage(): void {
    ...
    forkJoin([
      ...
      if (
        this.destinationField === QboExportSettingDestinationOptionKey.CREDIT_CARD_ACCOUNT &&
        this.cccExpenseObject === QBOCorporateCreditCardExpensesObject.DEBIT_CARD_EXPENSE
      ) {
      ...
      this.isMultiLineOption = responses[2].workspace_general_settings.import_code_fields?.includes(
      ...
      this.mappingService
        .getPaginatedDestinationAttributes(this.destinationAttributes, undefined, this.displayName)
        .subscribe((responses) => {
