...
export class NetsuiteDashboardComponent implements OnInit, OnDestroy {
  ...
  accountingExportType: NetsuiteTaskLogType[] = [
    NetsuiteTaskLogType.CREATING_BILL,
    NetsuiteTaskLogType.CREATING_EXPENSE_REPORT,
    NetsuiteTaskLogType.CREATING_CREDIT_CARD_CHARGE,
    NetsuiteTaskLogType.CREATING_CREDIT_CARD_REFUND,
    NetsuiteTaskLogType.CREATING_JOURNAL_ENTRY,
  ];
  ...
  private setupPage(): void {
    forkJoin([
      ...
      this.destinationFieldMap = {
        ...
        CATEGORY:
          responses[4].configuration.reimbursable_expenses_object ===
            NetsuiteReimbursableExpensesObject.EXPENSE_REPORT ||
          responses[4].configuration.corporate_credit_card_expenses_object ===
            NetSuiteCorporateCreditCardExpensesObject.EXPENSE_REPORT
      ...
      const queuedTasks: NetsuiteTaskLog[] = responses[2].results.filter(
        (task: NetsuiteTaskLog) => task.status === TaskLogState.ENQUEUED || task.status === TaskLogState.IN_PROGRESS,
      );
      this.failedExpenseGroupCount = responses[2].results.filter(
        (task: NetsuiteTaskLog) => task.status === TaskLogState.FAILED || task.status === TaskLogState.FATAL,
      ).length;

      this.exportableAccountingExportIds = responses[3].exportable_expense_group_ids;

      this.reimbursableImportState = responses[4].configuration.reimbursable_expenses_object
        ? this.reimbursableExpenseImportStateMap[responses[4].expense_group_settings.expense_state]
        : null;
      this.cccImportState = responses[4].configuration.corporate_credit_card_expenses_object
        ? this.cccExpenseImportStateMap[responses[4].expense_group_settings.ccc_expense_state]
        : null;
