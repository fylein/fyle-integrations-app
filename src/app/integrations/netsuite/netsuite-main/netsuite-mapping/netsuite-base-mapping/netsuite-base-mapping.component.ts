...
export class NetsuiteBaseMappingComponent implements OnInit {
    ...
    this.mappingService.triggerAutoMapEmployees().subscribe(
      () => {
        ...
        this.toastService.displayToastMessage(
          ToastSeverity.INFO,
          this.translocoService.translate('netsuiteBaseMapping.autoMappingInProgress'),
        );
      },
      () => {
        this.isLoading = false;
        this.toastService.displayToastMessage(
          ToastSeverity.ERROR,
          this.translocoService.translate('netsuiteBaseMapping.autoMappingError'),
        );
      },
    );
  }
  ...
  private getDestinationField(
    workspaceGeneralSetting: NetsuiteExportSettingWorkspaceGeneralSettingPost,
    ...
    if (this.sourceField === FyleField.EMPLOYEE) {
      ...
      return workspaceGeneralSetting.reimbursable_expenses_object ===
  ...
  private setupPage(): void {
    ...
    forkJoin([this.workspaceService.getConfiguration(), this.mappingService.getMappingSettings()]).subscribe(
      (responses) => {
        ...
        this.mappingService
          .getPaginatedDestinationAttributes(this.destinationField, undefined, this.displayName)
          .subscribe((responses) => {
