...
export class NetsuiteExportSettingsComponent implements OnInit {
  ...
  constructor(
    ...
    this.reimbursableExpenseGroupingDateOptions =
      this.netsuiteExportSettingsService.getReimbursableExpenseGroupingDateOptions();
    ...
    this.exportSettingForm.controls.reimbursableExportType.valueChanges
      ...
      .subscribe((selectedReimbursableExportType) => {
        ...
        this.creditCardExportTypes =
          this.netsuiteExportSettingsService.getCreditCardExportTypes(selectedReimbursableExportType);

        // If an invalid CCC export type was selected, reset it to null
        if (
          !this.creditCardExportTypes
            .map((option) => option.value)
            .includes(this.exportSettingForm.controls.creditCardExportType.value)
        ) {
  ...
  private cccExportTypeWatcher(): void {
    this.exportSettingForm.controls.creditCardExportType.valueChanges.subscribe((isCCCExportTypeSelected) => {
      if (
        isCCCExportTypeSelected === NetSuiteCorporateCreditCardExpensesObject.JOURNAL_ENTRY &&
        !this.exportSettings?.configuration?.reimbursable_expenses_object
      ) {
        this.exportSettingForm.controls.employeeFieldMapping.enable();
  ...
  private exportFieldsWatcher(): void {
    if (
      ...
      (this.exportSettings?.configuration?.corporate_credit_card_expenses_object ===
        NetSuiteCorporateCreditCardExpensesObject.JOURNAL_ENTRY &&
        !this.exportSettings?.configuration?.reimbursable_expenses_object)
    ) {
  ...
  getEmployeeFieldMapping(employeeFieldMapping: FyleField | null, reimbursableExportType: string): string {
    ...
    if (employeeFieldMapping) {
      ...
      employeeFieldMappingLabel =
        reimbursableExportType === NetsuiteReimbursableExpensesObject.EXPENSE_REPORT
          ? FyleField.EMPLOYEE
          : FyleField.VENDOR;
    }
  ...
  constructPayloadAndSave(data: ConfigurationWarningOut): void {
    ...
    if (data.hasAccepted) {
      ...
      this.exportSettingService.postExportSettings(exportSettingPayload).subscribe(
        (response: NetSuiteExportSettingGet) => {
          this.isSaveInProgress = false;
          this.toastService.displayToastMessage(
            ToastSeverity.SUCCESS,
            this.translocoService.translate('netsuiteExportSettings.exportSettingsSaved'),
            undefined,
            this.isOnboarding,
          );
        ...
        () => {
          ...
          this.toastService.displayToastMessage(
            ToastSeverity.ERROR,
            this.translocoService.translate('netsuiteExportSettings.exportSettingsError'),
          );
        },
      );
    }
  ...
  private isAdvancedSettingAffected(): boolean {
    return (
      (this.exportSettings?.configuration?.reimbursable_expenses_object !== null &&
        this.exportSettings?.configuration?.reimbursable_expenses_object !==
          NetsuiteReimbursableExpensesObject.JOURNAL_ENTRY &&
        this.exportSettingForm.value.reimbursableExportType === NetsuiteReimbursableExpensesObject.JOURNAL_ENTRY) ||
      (this.exportSettings?.configuration?.corporate_credit_card_expenses_object !== null &&
        this.exportSettings?.configuration?.corporate_credit_card_expenses_object !==
          NetSuiteCorporateCreditCardExpensesObject.JOURNAL_ENTRY &&
  ...
  private replaceContentBasedOnConfiguration(
    ...
    if (updatedConfiguration && existingConfiguration) {
      const translatedExportType = this.translocoService.translate(
        exportType === 'reimbursable' ? 'netsuiteExportSettings.reimbursable' : 'netsuiteExportSettings.creditCard',
      );
  ...
  private constructWarningMessage(): string {
    ...
    const updatedReimbursableExportType = this.exportSettingForm.value.reimbursableExportType
      ? this.exportSettingForm.value.reimbursableExportType
      : null;
    const updatedCorporateCardExportType = this.exportSettingForm.value.creditCardExportType
      ? this.exportSettingForm.value.creditCardExportType
      : null;
    ...
    this.exportSettingService.creditCardExportTypeChange.subscribe(
      (selectedValue: NetSuiteCorporateCreditCardExpensesObject) => {
        this.showNameInJournalOption =
          selectedValue === NetSuiteCorporateCreditCardExpensesObject.JOURNAL_ENTRY ? true : false;
  ...
  addMissingOptions() {
    if (this.exportSettings.general_mappings) {
      this.helperService.addDefaultDestinationAttributeIfNotExists({
        ...
        newOption: this.exportSettings.general_mappings.reimbursable_account,
      });
      this.helperService.addDefaultDestinationAttributeIfNotExists({
      ...
      this.helperService.addDefaultDestinationAttributeIfNotExists({
        options: this.creditCardVendors,
  ...
  private getSettingsAndSetupForm(): void {
    ...
    forkJoin([this.exportSettingService.getExportSettings(), ...groupedAttributes]).subscribe(
      ([exportSetting, vendors, accountsPayables, bankAccounts, cccAccounts]) => {
        this.exportSettings = exportSetting;

        this.creditCardVendors = vendors.results.map((option) =>
          this.exportSettingsService.formatGeneralMappingPayload(option),
        );
        this.accountsPayables = accountsPayables.results.map((option) =>
          this.exportSettingsService.formatGeneralMappingPayload(option),
        );
        this.bankAccounts = bankAccounts.results.map((option) =>
          this.exportSettingsService.formatGeneralMappingPayload(option),
        );
        ...
        if (
          this.exportSettings.configuration &&
          this.exportSettings.configuration.corporate_credit_card_expenses_object
        ) {
          this.exportSettingService.setupDynamicValidators(
            this.exportSettingForm,
