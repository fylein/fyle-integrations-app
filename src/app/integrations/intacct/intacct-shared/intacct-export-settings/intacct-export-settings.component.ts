...
import {
  ExportSettingDestinationAttributeOption,
...
import {
  ExportSettingFormOption,
  ExportSettingGet,
  IntacctExportSettingOptionSearch,
} from 'src/app/core/models/intacct/intacct-configuration/export-settings.model';
...
export class IntacctExportSettingsComponent implements OnInit {
  ...
  refreshDimensions(isRefresh: boolean) {
    ...
    this.toastService.displayToastMessage(
      ...
      this.translocoService.translate('intacctExportSettings.syncDataDimensionsToast'),
    );
  }
  ...
  getEmployeeFieldMapping(employeeFieldMapping: FyleField | null, reimbursableExportType: string): string {
    ...
    if (employeeFieldMapping) {
      ...
      employeeFieldMappingLabel =
        reimbursableExportType === IntacctReimbursableExpensesObject.EXPENSE_REPORT
          ? FyleField.EMPLOYEE
          : FyleField.VENDOR;
    }
  ...
  getExportType(
    exportType: IntacctReimbursableExpensesObject | IntacctCorporateCreditCardExpensesObject | null,
  ): string {
    if (exportType) {
  ...
  private isAdvancedSettingAffected(): boolean {
    return (
      (this.exportSettings?.configurations?.reimbursable_expenses_object !== null &&
        this.exportSettings?.configurations?.reimbursable_expenses_object !==
          IntacctReimbursableExpensesObject.JOURNAL_ENTRY &&
        this.exportSettingsForm.value.reimbursableExportType === IntacctReimbursableExpensesObject.JOURNAL_ENTRY) ||
      (this.exportSettings?.configurations?.corporate_credit_card_expenses_object !== null &&
        this.exportSettings?.configurations?.corporate_credit_card_expenses_object !==
          IntacctCorporateCreditCardExpensesObject.JOURNAL_ENTRY &&
        this.exportSettingsForm.value.creditCardExportType === IntacctCorporateCreditCardExpensesObject.JOURNAL_ENTRY)
    );
  }

  private replaceContentBasedOnConfiguration(
    updatedConfiguration: string,
    existingConfiguration: string | undefined | null,
    exportType: string,
  ): string {
    let content = '';
    ...
    if (updatedConfiguration && existingConfiguration) {
      content = this.translocoService.translate('intacctExportSettings.advancedSettingsWarning', {
  ...
  private constructWarningMessage(): string {
    ...
    const updatedReimbursableExportType = this.exportSettingsForm.value.reimbursableExportType
      ? this.exportSettingsForm.value.reimbursableExportType
      : null;
    const updatedCorporateCardExportType = this.exportSettingsForm.value.creditCardExportType
      ? this.exportSettingsForm.value.creditCardExportType
      : null;
  ...
  private reimbursableExportTypeWatcher(): void {
    this.exportSettingsForm.controls.reimbursableExportType.valueChanges
      ...
      .subscribe((selectedReimbursableExportType) => {
        ...
        this.creditCardExportTypes = this.exportSettingsService.constructCCCOptions(
          brandingConfig.brandId,
        ...
        if (
          !this.creditCardExportTypes
            ...
            .includes(this.exportSettingsForm.controls.cccExportType.value)
        ) {
  ...
  private initializeExportSettingsFormWithData(): void {
    ...
    const findObjectById = (array: IntacctDestinationAttribute[], id: string | null) =>
      array?.find((item) => item.destination_id === id) || null;

    this.exportSettingsForm = this.formBuilder.group({
      reimbursableExpense: [
        Boolean(configurations?.reimbursable_expenses_object) || null,
        this.exportSelectionValidator(),
      ],
      reimbursableExportType: [configurations?.reimbursable_expenses_object || null],
      reimbursableExpensePaymentType: [
        findObjectById(
          this.destinationOptions.EXPENSE_PAYMENT_TYPE,
          generalMappings?.default_reimbursable_expense_payment_type.id,
        ),
      ],
      reimbursableExportGroup: [
        this.exportSettingsService.getExportGroup(
          this.exportSettings?.expense_group_settings.reimbursable_expense_group_fields,
        ) || null,
      ],
      ...
      creditCardExpense: [
        Boolean(configurations?.corporate_credit_card_expenses_object),
        this.exportSelectionValidator(),
      ],
      cccExportType: [configurations?.corporate_credit_card_expenses_object || null],
      cccExportGroup: [
        this.exportSettingsService.getExportGroup(
          this.exportSettings?.expense_group_settings.corporate_credit_card_expense_group_fields,
        ),
      ],
      ...
      cccExpensePaymentType: [
        findObjectById(
          this.destinationOptions.CCC_EXPENSE_PAYMENT_TYPE,
          generalMappings?.default_ccc_expense_payment_type.id,
        ),
      ],
      ...
      useMerchantInJournalLine: [
        brandingFeatureConfig.featureFlags.exportSettings.useMerchantInJournalLine
          ? configurations?.use_merchant_in_journal_line
            ? configurations?.use_merchant_in_journal_line
            : false
          : true,
      ],
  ...
  private addMissingOption(
    key: IntacctExportSettingDestinationOptionKey,
    defaultDestinationAttribute: DefaultDestinationAttribute,
  ): void {
  ...
  private addMissingOptions(): void {
    ...
    this.addMissingOption(
      IntacctExportSettingDestinationOptionKey.ACCOUNT,
      this.exportSettings.general_mappings?.default_gl_account,
    );
    this.addMissingOption(
      IntacctExportSettingDestinationOptionKey.ACCOUNT,
      this.exportSettings.general_mappings?.default_credit_card,
    );
    this.addMissingOption(
      IntacctExportSettingDestinationOptionKey.EXPENSE_PAYMENT_TYPE,
      this.exportSettings.general_mappings?.default_reimbursable_expense_payment_type,
    );
    this.addMissingOption(
      IntacctExportSettingDestinationOptionKey.CCC_EXPENSE_PAYMENT_TYPE,
      this.exportSettings.general_mappings?.default_ccc_expense_payment_type,
    );
    this.addMissingOption(
      IntacctExportSettingDestinationOptionKey.VENDOR,
      this.exportSettings.general_mappings?.default_ccc_vendor,
    );
    this.addMissingOption(
      IntacctExportSettingDestinationOptionKey.CHARGE_CARD,
      this.exportSettings.general_mappings?.default_charge_card,
    );
  }
  ...
  constructPayloadAndSave(data: ConfigurationWarningOut): void {
    ...
    if (data.hasAccepted) {
      ...
      this.exportSettingService.postExportSettings(exportSettingPayload).subscribe(
        (response: ExportSettingGet) => {
          this.toastService.displayToastMessage(
            ToastSeverity.SUCCESS,
            this.translocoService.translate('intacctExportSettings.exportSettingsSuccess'),
            undefined,
            this.isOnboarding,
          );
          ...
          if (this.workspaceService.getIntacctOnboardingState() === IntacctOnboardingState.EXPORT_SETTINGS) {
            this.trackingService.integrationsOnboardingCompletion(
              TrackingApp.INTACCT,
              IntacctOnboardingState.EXPORT_SETTINGS,
              2,
              exportSettingPayload,
            );
          } else {
            this.trackingService.intacctUpdateEvent(IntacctUpdateEvent.IMPORT_SETTINGS_INTACCT, {
              phase: this.getPhase(),
        ...
        () => {
          ...
          this.toastService.displayToastMessage(
            ToastSeverity.ERROR,
            this.translocoService.translate('intacctExportSettings.exportSettingsError'),
          );
        },
      );
    }
  ...
  private setupCustomWatchers(): void {
    ...
    this.exportSettingsForm.controls.cccExportGroup?.valueChanges.subscribe((cccExportGroup) => {
      const isCoreCCCModule =
        this.exportSettingsForm?.value.cccExportType ===
        IntacctCorporateCreditCardExpensesObject.CHARGE_CARD_TRANSACTION;
      this.cccExpenseGroupingDateOptions = this.exportSettingsService.constructExportDateOptions(
        isCoreCCCModule,
        cccExportGroup,
        this.exportSettingsForm.controls.cccExportDate.value,
      );
  ...
  private sortDropdownOptions(destinationOptionKey: IntacctExportSettingDestinationOptionKey): void {
    this.destinationOptions[destinationOptionKey].sort(
      (a: IntacctDestinationAttribute, b: IntacctDestinationAttribute) => {
  ...
  private optionSearchWatcher(): void {
    this.optionSearchUpdate.pipe(debounceTime(1000)).subscribe((event: IntacctExportSettingOptionSearch) => {
      ...
      this.mappingService
        .getPaginatedDestinationAttributes(existingOptions[0].attribute_type, event.searchTerm)
        .subscribe((response) => {
          response.results.forEach((option) => {
            ...
            if (event.destinationOptionKey === IntacctExportSettingDestinationOptionKey.EXPENSE_PAYMENT_TYPE) {
              ...
              event.destinationOptionKey === IntacctExportSettingDestinationOptionKey.CCC_EXPENSE_PAYMENT_TYPE
            ) {
              if (!option.detail.is_reimbursable) {
                newOptions.push(option);
              }
  ...
  private setupPage(): void {
    ...
    forkJoin(groupedAttributes$).subscribe((response) => {
      ...
      this.destinationOptions.EXPENSE_PAYMENT_TYPE = response[1].results.filter(
        (attr: IntacctDestinationAttribute) => attr.detail.is_reimbursable,
      );
      this.destinationOptions.CCC_EXPENSE_PAYMENT_TYPE = response[1].results.filter(
        (attr: IntacctDestinationAttribute) => !attr.detail.is_reimbursable,
      );
