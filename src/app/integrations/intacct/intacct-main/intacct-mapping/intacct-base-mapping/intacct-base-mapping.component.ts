...
export class IntacctBaseMappingComponent implements OnInit {
    ...
    this.mappingService.triggerAutoMapEmployees().subscribe(
      () => {
        ...
        this.toastService.displayToastMessage(
          ToastSeverity.INFO,
          this.translocoService.translate('intacctBaseMapping.autoMappingInfo'),
        );
      },
      () => {
        this.isLoading = false;
        this.toastService.displayToastMessage(
          ToastSeverity.ERROR,
          this.translocoService.translate('intacctBaseMapping.somethingWentWrong'),
        );
      },
    );
  }
  ...
  private getDestinationField(intacctConfiguration: IntacctConfiguration, mappingSettings: MappingSetting[]): string {
    if (this.sourceField === FyleField.EMPLOYEE) {
      ...
      if (
        intacctConfiguration.reimbursable_expenses_object === IntacctReimbursableExpensesObject.EXPENSE_REPORT ||
        intacctConfiguration.corporate_credit_card_expenses_object ===
  ...
  private setupPage(): void {
    ...
    forkJoin([this.workspaceService.getConfiguration(), this.mappingService.getMappingSettings()]).subscribe(
      (responses) => {
        ...
        if (this.sourceField === FyleField.CORPORATE_CARD && this.destinationField === SageIntacctField.VENDOR) {
          this.destinationFieldDisplayName = this.translocoService.translate('intacctBaseMapping.creditCardVendor');
        }

        this.mappingService
          .getPaginatedDestinationAttributes(this.destinationField, undefined)
