...
import {
  ...
  BusinessCentralAdvancedSettingsModel,
...
export class BusinessCentralAdvancedSettingsComponent implements OnInit {
      ...
      if (this.skipExportForm.controls.condition1.value) {
        if (this.skipExportForm.controls.condition2.value) {
          return (this.skipExportForm.controls.value1.value ||
            ...
            this.skipExportForm.controls.operator1.value === CustomOperatorOption.IsNotEmpty) &&
            (this.skipExportForm.controls.value2.value ||
              this.skipExportForm.controls.operator2.value === CustomOperatorOption.IsEmpty ||
              this.skipExportForm.controls.operator2.value === CustomOperatorOption.IsNotEmpty)
            ? true
            : false;
        }
        return this.skipExportForm.controls.value1.value ||
  ...
  constructPayloadAndSave() {
    ...
    const advancedSettingPayload = BusinessCentralAdvancedSettingsModel.createAdvancedSettingPayload(
      this.advancedSettingForm,
    );
    this.advancedSettingsService.postAdvancedSettings(advancedSettingPayload).subscribe(
      (advancedSettingsResponse: BusinessCentralAdvancedSettingsGet) => {
        this.isSaveInProgress = false;
        this.toastService.displayToastMessage(
          ToastSeverity.SUCCESS,
          this.translocoService.translate('businessCentralAdvancedSettings.saveSuccessToast'),
        );
        this.trackingService.trackTimeSpent(
          TrackingApp.BUSINESS_CENTRAL,
          Page.ADVANCED_SETTINGS_BUSINESS_CENTRAL,
      ...
      () => {
        ...
        this.toastService.displayToastMessage(
          ToastSeverity.ERROR,
          this.translocoService.translate('businessCentralAdvancedSettings.saveErrorToast'),
        );
      },
    );
  }
  ...
  private getSettingsAndSetupForm(): void {
    ...
    forkJoin([
      ...
      this.advancedSettingForm = BusinessCentralAdvancedSettingsModel.mapAPIResponseToFormGroup(
