...
export class Sage300Component implements OnInit {
  ...
  private navigate(isSage300TokenValid?: boolean): void {
    ...
    if (pathName === '/integrations/sage300') {
      ...
      this.router.navigateByUrl(
        isSage300TokenValid === false &&
          ![Sage300OnboardingState.CONNECTION, Sage300OnboardingState.COMPLETE].includes(
            this.workspace.onboarding_state,
          )
          ? onboardingStateComponentMap[Sage300OnboardingState.CONNECTOR_AUTH]
  ...
  private routeBasedOnTokenStatus(): void {
    this.sage300Connector.getSage300TokenHealthStatus().subscribe((isSage300TokenValid) => {
