...
export class Sage300ExportSettingsComponent implements OnInit {
  ...
  reimbursableExpenseGroupingDateOptions: SelectFormOption[] =
    this.exportSettingService.getReimbursableExpenseGroupingDateOptions();

  cccExpenseGroupingDateOptions: SelectFormOption[] = this.exportSettingService.getCCCExpenseGroupingDateOptions();

  expensesExportTypeOptions: Sage300ExportSettingFormOption[] =
    this.exportSettingService.getExpensesExportTypeOptions();
  ...
  private setupCustomWatchers(): void {
    this.exportSettingForm.controls.reimbursableExportGroup?.valueChanges.subscribe((reimbursableExportGroup) => {
      this.reimbursableExpenseGroupingDateOptions =
        this.exportSettingService.getReimbursableExpenseGroupingDateOptions();
      this.reimbursableExpenseGroupingDateOptions = this.exportSettingsService.constructGroupingDateOptions(
    ...
    this.exportSettingForm.controls.cccExportGroup?.valueChanges.subscribe((cccExportGroup) => {
      ...
      this.cccExpenseGroupingDateOptions = this.exportSettingsService.constructGroupingDateOptions(
        cccExportGroup,
      ...
      this.exportSettingsService.clearInvalidDateOption(this.exportSettingForm.get('cccExportDate'), validOptions);
    });
  }
  ...
  private constructPayloadAndSave(): void {
    ...
    this.exportSettingService.postExportSettings(exportSettingPayload).subscribe(
      (exportSettingResponse: Sage300ExportSettingGet) => {
        this.isSaveInProgress = false;
        this.toastService.displayToastMessage(
          ToastSeverity.SUCCESS,
          this.translocoService.translate('sage300ExportSettings.exportSettingsSavedSuccess'),
          undefined,
          this.isOnboarding,
        );
        ...
        if (this.workspaceService.getOnboardingState() === Sage300OnboardingState.EXPORT_SETTINGS) {
          this.trackingService.onOnboardingStepCompletion(
            TrackingApp.SAGE300,
            Sage300OnboardingState.EXPORT_SETTINGS,
            2,
      ...
      () => {
        ...
        this.toastService.displayToastMessage(
          ToastSeverity.ERROR,
          this.translocoService.translate('sage300ExportSettings.errorSavingExportSettings'),
        );
      },
    );
  }
  ...
  private optionSearchWatcher(): void {
    this.optionSearchUpdate.pipe(debounceTime(1000)).subscribe((event: ExportSettingOptionSearch) => {
      ...
      this.mappingService
        .getPaginatedDestinationAttributes(attribuiteType, event.searchTerm, '', this.appName)
        .subscribe((response) => {
