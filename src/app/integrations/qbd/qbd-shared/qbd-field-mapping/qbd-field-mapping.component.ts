...
export class QbdFieldMappingComponent implements OnInit {
  ...
  mappingFieldFormOptionsFunction(
    formControllerName1: string,
    formControllerName2: string,
  ): QBDExportSettingFormOption[] {
    const filteredOptions = this.representationOption.filter((option) => {
      return (
        option.value !== this.fieldMappingForm.get([formControllerName1])?.value &&
  ...
  constructPayloadAndSave(): void {
    ...
    this.fieldMappingService.postQbdFieldMapping(fieldMappingPayload).subscribe(
      (response: QBDFieldMappingGet) => {
        this.saveInProgress = false;
        this.toastService.displayToastMessage(
          ToastSeverity.SUCCESS,
          this.translocoService.translate('qbdFieldMapping.fieldMappingSuccess'),
        );
        ...
        if (this.workspaceService.getOnboardingState() === QBDOnboardingState.FIELD_MAPPINGS) {
          this.trackingService.onOnboardingStepCompletion(
            TrackingApp.QBD,
            QBDOnboardingState.FIELD_MAPPINGS,
            3,
            fieldMappingPayload,
          );
        } else {
      ...
      () => {
        ...
        this.toastService.displayToastMessage(
          ToastSeverity.ERROR,
          this.translocoService.translate('qbdFieldMapping.fieldMappingError'),
        );
      },
    );
  }
  ...
  private getSettingsAndSetupForm(): void {
    ...
    this.fieldMappingService.getQbdFieldMapping().subscribe(
      (fieldMappingResponse: QBDFieldMappingGet) => {
