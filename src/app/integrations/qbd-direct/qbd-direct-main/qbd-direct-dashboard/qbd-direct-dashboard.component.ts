...
export class QbdDirectDashboardComponent implements OnInit, OnDestroy {
  ...
  exportLogProcessingStates: TaskLogState[] = [
    TaskLogState.IN_PROGRESS,
    TaskLogState.ENQUEUED,
    TaskLogState.EXPORT_PROCESSED,
  ];
  ...
  private pollExportStatus(exportableAccountingExportIds: number[] = []): void {
    interval(3000)
      .pipe(
        switchMap(() =>
          from(this.dashboardService.getAllTasks([], exportableAccountingExportIds, [], AppName.QBD_DIRECT)),
        ),
        takeUntil(this.destroy$),
        takeWhile(
          (response: QbdDirectTaskResponse) =>
            response.results.filter((task) => this.exportLogProcessingStates.includes(task.status)).length > 0,
          true,
        ),
      )
      .subscribe((res: QbdDirectTaskResponse) => {
        this.processedCount = res.results.filter(
          (task: { status: TaskLogState }) =>
            task.status && task.status !== TaskLogState.IN_PROGRESS && task.status !== TaskLogState.ENQUEUED,
        ).length;
        ...
        if (
          res.results.filter((task: { status: TaskLogState }) => this.exportLogProcessingStates.includes(task.status))
            .length === 0
        ) {
          forkJoin([this.getExportErrors$, this.getAccountingExportSummary$]).subscribe((responses) => {
            ...
            this.accountingExportSummary = AccountingExportSummaryModel.parseAPIResponseToAccountingSummaryForQbdDirect(
              responses[1],
            );
            this.failedExpenseGroupCount = res.results.filter(
              (task) => task.status === TaskLogState.ERROR || task.status === TaskLogState.FATAL,
            ).length;

            this.exportableAccountingExportIds = res.results
              .filter((task) => task.status === TaskLogState.ERROR || task.status === TaskLogState.FATAL)
              .map((taskLog) => taskLog.id);
            ...
            if (this.failedExpenseGroupCount === 0) {
              this.refinerService.triggerSurvey(
                AppName.QBD_DIRECT,
                environment.refiner_survey.qbd.export_done_survery_id,
  ...
  private setupPage(): void {
    forkJoin([
      ...
      this.dashboardService.getAllTasks(
        this.exportLogProcessingStates.concat(TaskLogState.ERROR),
        undefined,
        [],
        AppName.QBD_DIRECT,
      ),
      ...
      this.chartOfAccounts = responses[5].import_settings.import_account_as_category
        ? responses[5].import_settings.chart_of_accounts.map((item: string) => item.replace(/\s+/g, ''))
        : this.importSettingService.getChartOfAccountTypesList().map((item: string) => item.replace(/\s+/g, ''));

      const queuedTasks: QbdDirectTaskLog[] = responses[2].results.filter((task: QbdDirectTaskLog) =>
        this.exportLogProcessingStates.includes(task.status),
      );
      this.failedExpenseGroupCount = responses[2].results.filter(
        (task: QbdDirectTaskLog) => task.status === TaskLogState.ERROR || task.status === TaskLogState.FATAL,
      ).length;

      this.exportableAccountingExportIds = responses[3].exportable_export_log_ids?.length
        ? responses[3].exportable_export_log_ids
        : [];

      this.reimbursableImportState =
        responses[4]?.reimbursable_expense_export_type && responses[4].reimbursable_expense_state
          ? this.reimbursableExpenseImportStateMap[responses[4].reimbursable_expense_state]
          : null;
      this.cccImportState =
        responses[4].credit_card_expense_export_type && responses[4].credit_card_expense_state
          ? this.cccExpenseImportStateMap[responses[4].credit_card_expense_state]
          : null;
      ...
      if (queuedTasks.length) {
        ...
        this.accountingExportService.importExpensesFromFyle('v2').subscribe(() => {
          this.dashboardService.getExportableAccountingExportIds('v2').subscribe((exportableAccountingExportIds) => {
            this.exportableAccountingExportIds = exportableAccountingExportIds.exportable_export_log_ids?.length
              ? exportableAccountingExportIds.exportable_export_log_ids
              : [];
            this.isImportInProgress = false;
          });
        });
