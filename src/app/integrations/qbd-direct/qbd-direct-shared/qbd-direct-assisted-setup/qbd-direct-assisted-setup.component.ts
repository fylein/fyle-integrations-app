...
export class QbdDirectAssistedSetupComponent {
  ...
  get isButtonDisabled(): boolean {
    return (
      this.interactionType === QBDDirectInteractionType.QUERY && !this.issueDescription.trim() && !this.isQuerySubmitted
    );
  }
    ...
    this.assistedSetupService.submitRequest(this.issueDescription).subscribe({
      ...
      error: () => {
        this.toastService.displayToastMessage(
          ToastSeverity.ERROR,
          this.translocoService.translate('qbdDirectAssistedSetup.failedToSubmitRequest'),
        );
      },
    });
  }
    ...
    this.assistedSetupService.bookSlot().subscribe({
      next: () => {
        ...
        this.toastService.displayToastMessage(
          ...
          this.translocoService.translate('qbdDirectAssistedSetup.requestReceived'),
        );
      ...
      error: () => {
        this.toastService.displayToastMessage(
          ToastSeverity.ERROR,
          this.translocoService.translate('qbdDirectAssistedSetup.somethingWentWrong'),
        );
      },
    });
  }
