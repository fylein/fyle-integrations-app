...
export class QbdDirectOnboardingPreRequisiteComponent {
  ...
  QBDconnectorArticleLink: string =
    brandingConfig.brandId === 'co' ? '' : brandingKbArticles.onboardingArticles.QBD_DIRECT.CONNECTOR;
  ...
  constructor(
    ...
    this.preRequisitesteps = [
      ...
      },
      {
        ...
        caption: this.translocoService.translate('qbdDirectOnboardingPreRequisite.keepCompanyFileOpenCaption', {
    ...
    this.onboardingSteps = this.onboardingService.getOnboardingSteps(
      this.translocoService.translate('qbd_direct.configuration.preRequisite.stepName'),
  ...
  updateConnectorStatus(status: CheckBoxUpdate): void {
    this.preRequisitesteps[status.id - 1].state = status.value
      ? QBDPreRequisiteState.COMPLETE
      : QBDPreRequisiteState.INCOMPLETE;
    if (
      this.preRequisitesteps[0].state === QBDPreRequisiteState.COMPLETE &&
      this.preRequisitesteps[1].state === QBDPreRequisiteState.COMPLETE &&
      this.preRequisitesteps[2].state === QBDPreRequisiteState.COMPLETE
    ) {
    ...
    this.workspaceService
      ...
      .subscribe((workspaceResponse: QbdDirectWorkspace) => {
        this.trackingService.trackTimeSpent(
          ...
          Page.CONFIRM_PRE_REQUISITES_QBD_DIRECT,
        ...
        if (this.workspaceService.getOnboardingState() === QbdDirectOnboardingState.CONFIRM_PRE_REQUISITES) {
          this.trackingService.integrationsOnboardingCompletion(
            TrackingApp.QBD_DIRECT,
            QbdDirectOnboardingState.CONFIRM_PRE_REQUISITES,
            1,
          );
        } else {
