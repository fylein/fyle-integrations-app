...
import {
  QbdConnectorGet,
  SyncDataType,
} from 'src/app/core/models/qbd-direct/qbd-direct-configuration/qbd-direct-connector.model';
...
export class QbdDirectOnboardingConnectorComponent implements OnInit {
  ...
  triggerDownload(filePath: string) {
    if (filePath) {
      ...
      this.qbdDirectConnectorService
        .postQbdDirectConntion({ file_location: filePath })
        .subscribe((connectionResponse: QbdConnectorGet) => {
  ...
  proceedToConnection() {
    ...
    this.workspaceService
      .updateWorkspaceOnboardingState({ onboarding_state: QbdDirectOnboardingState.PENDING_QWC_UPLOAD })
      .subscribe((workspaceResponse: QbdDirectWorkspace) => {
        ...
        if (this.workspaceService.getOnboardingState() === QbdDirectOnboardingState.CONNECTION) {
          this.trackingService.integrationsOnboardingCompletion(
            ...
            QbdDirectOnboardingState.CONNECTION,
            2,
          );
        } else {
          const oldWorkspaceResponse = workspaceResponse;
          oldWorkspaceResponse.onboarding_state = QbdDirectOnboardingState.CONNECTION;
          this.trackingService.onUpdateEvent(TrackingApp.QBD_DIRECT, QbdDirectUpdateEvent.CONNECT_QBD_DIRECT, {
    ...
    if (event.value) {
      interval(3000)
        .pipe(
          ...
          takeWhile(
            (status: any) => !this.isTerminalStatus(status[0].onboarding_state as QbdDirectOnboardingState),
            true,
  ...
  handleDataSyncState(status: QbdDirectWorkspace) {
    ...
    this.qbdDirectConnectorService.syncAttribuites().subscribe((qbdAttribuites: SyncDataType[]) => {
      ...
      this.isDataSyncCTADisabled =
        onboardingState === QbdDirectOnboardingState.DESTINATION_SYNC_COMPLETE ? false : true;
    });
  }
  ...
  handleStatus(status: QbdDirectWorkspace): void {
    ...
    if (onboardingState === QbdDirectOnboardingState.INCORRECT_COMPANY_PATH) {
      ...
      this.warningDialogText = this.translocoService.translate(
        'qbdDirectOnboardingConnector.incorrectCompanyPathMessage',
      );
      ...
      onboardingState === QbdDirectOnboardingState.DESTINATION_SYNC_IN_PROGRESS ||
      onboardingState === QbdDirectOnboardingState.DESTINATION_SYNC_COMPLETE
    ) {
  ...
  proceedToSyncData() {
    ...
    this.workspaceService.getWorkspace(this.user.org_id).subscribe((workspaceResponse: QbdDirectWorkspace[]) => {
      if (workspaceResponse[0].onboarding_state === QbdDirectOnboardingState.DESTINATION_SYNC_IN_PROGRESS) {
        interval(3000)
          .pipe(
            switchMap(() => this.workspaceService.getWorkspace(this.user.org_id)), // Make HTTP request
            takeWhile(
              (status: any) => !this.isTerminalStatusDataSync(status[0].onboarding_state as QbdDirectOnboardingState),
              true,
            ), // Stop if terminal status is reached
          )
          .subscribe(
      ...
      if (
        workspaceResponse[0].onboarding_state in
        [QbdDirectOnboardingState.DESTINATION_SYNC_IN_PROGRESS, QbdDirectOnboardingState.DESTINATION_SYNC_COMPLETE]
      ) {
        this.trackingService.integrationsOnboardingCompletion(
          TrackingApp.QBD_DIRECT,
        ...
        this.trackingService.onUpdateEvent(TrackingApp.QBD_DIRECT, QbdDirectUpdateEvent.PENDING_QWC_UPLOAD_QBD_DIRECT, {
          phase: ProgressPhase.ONBOARDING,
          oldState: oldWorkspaceResponse,
  ...
  closeDialog(event: any) {
    if (this.connectionStatus === QBDConnectionStatus.IN_CORRECT_PASSWORD) {
      ...
      this.workspaceService
        .updateWorkspaceOnboardingState({ onboarding_state: QbdDirectOnboardingState.CONNECTION })
        .subscribe((workspaceResponse: QbdDirectWorkspace) => {
          this.isDialogVisible = false;
          this.isConnectionStepCompleted = false;
  ...
  proceedToExportSetting() {
    this.isLoading = true;
    this.workspaceService
      .updateWorkspaceOnboardingState({ onboarding_state: QbdDirectOnboardingState.EXPORT_SETTINGS })
      .subscribe((workspaceResponse: QbdDirectWorkspace) => {
        ...
        this.toastService.displayToastMessage(
          ...
          this.translocoService.translate('qbdDirectOnboardingConnector.connectionSuccessToast'),
          undefined,
          true,
        );
      });
  ...
  setupPage() {
    this.workspaceService.getWorkspace(this.user.org_id).subscribe((workspaceResponse: QbdDirectWorkspace[]) => {
      if (workspaceResponse[0].onboarding_state === QbdDirectOnboardingState.PENDING_QWC_UPLOAD) {
        ...
        workspaceResponse[0].onboarding_state === QbdDirectOnboardingState.DESTINATION_SYNC_IN_PROGRESS ||
        workspaceResponse[0].onboarding_state === QbdDirectOnboardingState.DESTINATION_SYNC_COMPLETE
      ) {
        this.isDownloadStepCompleted = true;
        this.isConnectionStepCompleted = true;
        this.isConnectionLoading = false;
  ...
  ngOnInit(): void {
    this.onboardingSteps = this.onboardingService.getOnboardingSteps(
      this.translocoService.translate('qbd_direct.configuration.connector.stepName'),
