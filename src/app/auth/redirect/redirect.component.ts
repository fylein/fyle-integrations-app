  ...
  private setupNavigation(): void {
    if (this.route.snapshot.queryParams?.state) {
      if (this.route.snapshot.queryParams?.state.includes('travelperk_local_redirect')) {
        ...
        this.windowService.redirect(url);
      } else if (this.route.snapshot.queryParams?.state === 'intacct_local_redirect') {
        const url = `http://localhost:4200/oauth/default?code=${this.route.snapshot.queryParams.code}`;
        this.windowService.redirect(url);
      }
    } else {
