...
import {
  CustomSegment,
  NetsuiteImportSettingGet,
  NetsuiteImportSettingPost,
} from 'src/app/core/models/netsuite/netsuite-configuration/netsuite-import-setting.model';
...
export class NetsuiteImportSettingsService extends ImportSettingsService {
  ...
  mapAPIResponseToFormGroup(
    ...
    const expenseFieldsArray = importSettings?.mapping_settings
      ? this.constructFormArray(importSettings.mapping_settings, netsuiteFields)
      : [];
    ...
    return {
      configuration: {
        ...
        import_items: importSettingsForm.get('importItems')?.value
          ? importSettingsForm.get('importItems')?.value
          : false,
      ...
      general_mappings: {
        default_tax_code: importSettingsForm.get('defaultTaxCode')?.value
          ? this.netsuiteExportSettingsService.formatGeneralMappingPayload(
  ...
  postNetsuiteCustomSegments(customSegmentsPayload: CustomSegment): Observable<CustomSegment> {
    return this.apiService.post(
      `/workspaces/${this.workspaceService.getWorkspaceId()}/netsuite/custom_segments/`,
      customSegmentsPayload,
    );
  }
}
