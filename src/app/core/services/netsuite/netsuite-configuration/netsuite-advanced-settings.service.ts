...
import {
  NetsuiteAdvancedSettingGet,
  NetsuiteAdvancedSettingPost,
} from 'src/app/core/models/netsuite/netsuite-configuration/netsuite-advanced-settings.model';
...
export class NetsuiteAdvancedSettingsService extends AdvancedSettingsService {
  ...
  static setConfigurationSettingValidatorsAndWatchers(form: FormGroup): void {
    ...
    Object.values(validatorRule).forEach((value, index) => {
      form.controls[keys[index]].valueChanges.subscribe((selectedValue) => {
        if (
          selectedValue &&
          ((keys[index] === 'paymentSync' && selectedValue === NetsuitePaymentSyncDirection.FYLE_TO_NETSUITE) ||
            keys[index] !== 'paymentSync')
        ) {
  ...
  mapAPIResponseToFormGroup(
    advancedSettings: NetsuiteAdvancedSettingGet,
    isSkipExportEnabled: boolean,
    adminEmails: EmailOption[],
    shouldEnableAccountingPeriod: boolean,
    ...
    return new FormGroup({
      paymentSync: new FormControl(
        advancedSettings?.configuration.sync_fyle_to_netsuite_payments
          ? NetsuitePaymentSyncDirection.FYLE_TO_NETSUITE
          : advancedSettings?.configuration.sync_netsuite_to_fyle_payments
            ? NetsuitePaymentSyncDirection.NETSUITE_TO_FYLE
            : null,
      ),
      paymentAccount: new FormControl(
        advancedSettings?.general_mappings.vendor_payment_account?.id
          ? advancedSettings?.general_mappings.vendor_payment_account
          : null,
      ),
      netsuiteLocation: new FormControl(
        advancedSettings?.general_mappings.netsuite_location?.id
          ? advancedSettings?.general_mappings.netsuite_location
          : null,
      ),
      useEmployeeLocation: new FormControl(
        advancedSettings?.general_mappings.use_employee_location
          ? advancedSettings?.general_mappings.use_employee_location
          : false,
      ),
      netsuiteLocationLevel: new FormControl(
        advancedSettings?.general_mappings.netsuite_location_level
          ? findObjectByDestinationId(advancedSettings?.general_mappings.netsuite_location_level)
          : this.getDefaultLevelOptions()[0],
      ),
      netsuiteDepartment: new FormControl(
        advancedSettings?.general_mappings.netsuite_department?.id
          ? advancedSettings?.general_mappings.netsuite_department
          : null,
      ),
      netsuiteDepartmentLevel: new FormControl(
        advancedSettings?.general_mappings.netsuite_department_level
          ? findObjectByDestinationId(advancedSettings?.general_mappings.netsuite_department_level)
          : this.getDefaultLevelOptions()[0],
      ),
      useEmployeeDepartment: new FormControl(
        advancedSettings?.general_mappings.use_employee_department
          ? advancedSettings?.general_mappings.use_employee_department
          : false,
      ),
      netsuiteClass: new FormControl(
        advancedSettings?.general_mappings.netsuite_class?.id
          ? advancedSettings?.general_mappings.netsuite_class
          : null,
      ),
      netsuiteClassLevel: new FormControl(
        advancedSettings?.general_mappings.netsuite_class_level
          ? findObjectByDestinationId(advancedSettings?.general_mappings.netsuite_class_level)
          : this.getDefaultLevelOptions()[0],
      ),
      useEmployeeClass: new FormControl(
        advancedSettings?.general_mappings.use_employee_class
          ? advancedSettings?.general_mappings.use_employee_class
          : false,
      ),
      ...
      changeAccountingPeriod: new FormControl(
        advancedSettings?.configuration?.change_accounting_period ?? shouldEnableAccountingPeriod,
      ),
      ...
      exportSchedule: new FormControl(
        advancedSettings.workspace_schedules?.enabled ||
          (isOnboarding && brandingFeatureConfig.featureFlags.dashboard.useRepurposedExportSummary)
          ? true
          : false,
      ),
      exportScheduleFrequency: new FormControl(
        this.getExportFrequency(
          advancedSettings.workspace_schedules?.is_real_time_export_enabled,
          isOnboarding,
          advancedSettings.workspace_schedules?.enabled,
          advancedSettings.workspace_schedules?.interval_hours,
        ),
      ),
      memoStructure: new FormControl(advancedSettings?.configuration.memo_structure),
      autoCreateMerchants: new FormControl(
        advancedSettings?.configuration?.auto_create_merchants
          ? advancedSettings.configuration.auto_create_merchants
          : false,
      ),
      ...
      email: new FormControl(
        advancedSettings?.workspace_schedules?.emails_selected &&
          advancedSettings?.workspace_schedules?.emails_selected?.length > 0
          ? AdvancedSettingsService.filterAdminEmails(
              advancedSettings?.workspace_schedules?.emails_selected,
              adminEmails,
            )
          : [],
    ...
    const advancedSettingPayload: NetsuiteAdvancedSettingPost = {
      configuration: {
        sync_fyle_to_netsuite_payments:
          advancedSettingsForm.get('paymentSync')?.value &&
          advancedSettingsForm.get('paymentSync')?.value === NetsuitePaymentSyncDirection.FYLE_TO_NETSUITE
            ? true
            : false,
        sync_netsuite_to_fyle_payments:
          advancedSettingsForm.get('paymentSync')?.value &&
          advancedSettingsForm.get('paymentSync')?.value === NetsuitePaymentSyncDirection.NETSUITE_TO_FYLE
            ? true
            : false,
      ...
      general_mappings: {
        vendor_payment_account: advancedSettingsForm.get('paymentAccount')?.value
          ? advancedSettingsForm.get('paymentAccount')?.value
          : emptyDestinationAttribute,
        netsuite_location: advancedSettingsForm.get('netsuiteLocation')?.value
          ? advancedSettingsForm.get('netsuiteLocation')?.value
          : emptyDestinationAttribute,
        netsuite_location_level: advancedSettingsForm.get('netsuiteLocationLevel')?.value
          ? advancedSettingsForm.get('netsuiteLocationLevel')?.value.id
          : '',
        netsuite_department: advancedSettingsForm.get('netsuiteDepartment')?.value
          ? advancedSettingsForm.get('netsuiteDepartment')?.value
          : emptyDestinationAttribute,
        netsuite_department_level: advancedSettingsForm.get('netsuiteDepartmentLevel')?.value
          ? advancedSettingsForm.get('netsuiteDepartmentLevel')?.value.id
          : '',
        netsuite_class: advancedSettingsForm.get('netsuiteClass')?.value
          ? advancedSettingsForm.get('netsuiteClass')?.value
          : emptyDestinationAttribute,
        netsuite_class_level: advancedSettingsForm.get('netsuiteClassLevel')?.value
          ? advancedSettingsForm.get('netsuiteClassLevel')?.value.id
          : '',
      ...
      workspace_schedules: {
        ...
        interval_hours: Number.isInteger(advancedSettingsForm.get('exportScheduleFrequency')?.value)
          ? advancedSettingsForm.get('exportScheduleFrequency')!.value
          : null,
        is_real_time_export_enabled:
          advancedSettingsForm.get('exportSchedule')?.value &&
          advancedSettingsForm.get('exportScheduleFrequency')?.value === 0
            ? true
            : false,
        emails_selected: advancedSettingsForm.get('email')?.value
          ? AdvancedSettingsService.formatSelectedEmails(advancedSettingsForm.get('email')?.value)
          : null,
        additional_email_options: advancedSettingsForm.get('additionalEmails')?.value
          ? advancedSettingsForm.get('additionalEmails')?.value[0]
          : null,
      },
    };
  ...
  postAdvancedSettings(exportSettingsPayload: NetsuiteAdvancedSettingPost): Observable<NetsuiteAdvancedSettingGet> {
    return this.apiService.put(
      `/v2/workspaces/${this.workspaceService.getWorkspaceId()}/advanced_configurations/`,
