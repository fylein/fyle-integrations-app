...
import {
  NetsuiteConnectorGet,
  NetsuiteConnectorPost,
  NetsuiteSubsidiaryMappingPost,
} from 'src/app/core/models/netsuite/netsuite-configuration/netsuite-connector.model';
...
export class NetsuiteConnectorService {
  ...
  mapAPIResponseToNetSuiteConnectorFormGroup(netsuiiteConnection: NetsuiteConnectorGet | null): FormGroup {
    ...
    return new FormGroup({
      accountId: new FormControl(
        { value: netsuiiteConnection?.ns_account_id ? netsuiiteConnection?.ns_account_id : null, disabled: isDisabled },
        Validators.required,
      ),
  ...
  connectNetsuite(
  ...
  ): Observable<{ netsuiteSetupForm: FormGroup; isNetsuiteConnected: boolean }> {
    this.workspaceId = this.storageService.get('workspaceId');
    ...
    return this.postCredentials(connectorPayload).pipe(
      switchMap((response) => {
        if (!isReconnecting) {
          return this.mappingsService.refreshNetsuiteDimensions(['subsidiaries']).pipe(
            map(() => {
              return {
                netsuiteSetupForm: this.mapAPIResponseToNetSuiteConnectorFormGroup(response),
        ...
        this.toastService.displayToastMessage(
          ...
          this.translocoService.translate('services.netsuiteConnector.connectionReconnectedToast'),
          6000,
        );
        return of({
          netsuiteSetupForm: this.mapAPIResponseToNetSuiteConnectorFormGroup(response),
      ...
      catchError(() => {
        this.toastService.displayToastMessage(
          ToastSeverity.ERROR,
          this.translocoService.translate('services.netsuiteConnector.connectionErrorToast'),
        );
        return of({
          netsuiteSetupForm: this.mapAPIResponseToNetSuiteConnectorFormGroup(this.netsuiteCredential),
    ...
    return this.checkNetsuiteTokenHealth(workspaceId).pipe(
      ...
      catchError((error) => {
        if (error.error.message !== 'Netsuite credentials not found' && shouldShowTokenExpiredMessage) {
          this.toastService.displayToastMessage(
            ToastSeverity.ERROR,
            this.translocoService.translate('services.netsuiteConnector.connectionExpiredToast'),
            6000,
          );
        }
        return of(false);
