...
import {
  QBOImportSettingGet,
  QBOImportSettingPost,
} from 'src/app/core/models/qbo/qbo-configuration/qbo-import-setting.model';
...
export class QboImportSettingsService extends ImportSettingsService {
  ...
  getChartOfAccountTypesList(): string[] {
    return [
      this.translocoService.translate('services.qboImportSettings.expense'),
      this.translocoService.translate('services.qboImportSettings.otherExpense'),
      this.translocoService.translate('services.qboImportSettings.fixedAsset'),
      this.translocoService.translate('services.qboImportSettings.costOfGoodsSold'),
      this.translocoService.translate('services.qboImportSettings.currentLiability'),
      this.translocoService.translate('services.qboImportSettings.equity'),
      this.translocoService.translate('services.qboImportSettings.otherCurrentAsset'),
      this.translocoService.translate('services.qboImportSettings.otherCurrentLiability'),
      this.translocoService.translate('services.qboImportSettings.longTermLiability'),
      this.translocoService.translate('services.qboImportSettings.currentAsset'),
      this.translocoService.translate('services.qboImportSettings.income'),
      this.translocoService.translate('services.qboImportSettings.otherIncome'),
    ];
  }
  ...
  mapAPIResponseToFormGroup(
    ...
    return new FormGroup({
      ...
      chartOfAccountTypes: new FormControl(
        importSettings?.workspace_general_settings.charts_of_accounts
          ? importSettings.workspace_general_settings.charts_of_accounts
          : [this.translocoService.translate('services.qboImportSettings.expense')],
      ),
      ...
      importVendorsAsMerchants: new FormControl(
        importSettings?.workspace_general_settings.import_vendors_as_merchants ?? false,
      ),
      defaultTaxCode: new FormControl(
        importSettings?.general_mappings?.default_tax_code?.id
          ? importSettings.general_mappings.default_tax_code
          : null,
      ),
    ...
    return {
      ...
      general_mappings: {
        default_tax_code: importSettingsForm.get('defaultTaxCode')?.value
          ? importSettingsForm.get('defaultTaxCode')?.value
