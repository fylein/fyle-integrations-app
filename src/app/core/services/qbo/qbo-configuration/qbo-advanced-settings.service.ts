...
import {
  QBOAdvancedSettingGet,
  QBOAdvancedSettingPost,
} from 'src/app/core/models/qbo/qbo-configuration/qbo-advanced-setting.model';
...
export class QboAdvancedSettingsService extends AdvancedSettingsService {
  ...
  getPaymentSyncOptions(): SelectFormOption[] {
    return [
      {
        label: this.translocoService.translate('services.qboAdvancedSettings.exportFyleACHPaymentsToQBO', {
          brandName: brandingConfig.brandName,
        }),
        value: QBOPaymentSyncDirection.FYLE_TO_QBO,
      },
      {
        label: this.translocoService.translate('services.qboAdvancedSettings.importQBOPaymentsToFyle', {
          brandName: brandingConfig.brandName,
        }),
  ...
  static setConfigurationSettingValidatorsAndWatchers(form: FormGroup): void {
    ...
    Object.values(validatorRule).forEach((value, index) => {
      form.controls[keys[index]].valueChanges.subscribe((selectedValue) => {
        if (
          selectedValue &&
          ((keys[index] === 'paymentSync' && selectedValue === QBOPaymentSyncDirection.FYLE_TO_QBO) ||
  ...
  mapAPIResponseToFormGroup(
    advancedSettings: QBOAdvancedSettingGet,
    ...
    return new FormGroup({
      paymentSync: new FormControl(
        advancedSettings?.workspace_general_settings.sync_fyle_to_qbo_payments
          ? QBOPaymentSyncDirection.FYLE_TO_QBO
          : advancedSettings?.workspace_general_settings.sync_qbo_to_fyle_payments
            ? QBOPaymentSyncDirection.QBO_TO_FYLE
            : null,
      ),
      billPaymentAccount: new FormControl(
        advancedSettings?.general_mappings.bill_payment_account?.id
          ? advancedSettings?.general_mappings.bill_payment_account
          : null,
      ),
      ...
      autoCreateMerchantsAsVendors: new FormControl(
        advancedSettings?.workspace_general_settings.auto_create_merchants_as_vendors,
      ),
    ...
    const advancedSettingPayload: QBOAdvancedSettingPost = {
      workspace_general_settings: {
        sync_fyle_to_qbo_payments:
          ...
          advancedSettingsForm.get('paymentSync')?.value === QBOPaymentSyncDirection.FYLE_TO_QBO
            ? true
            : false,
        sync_qbo_to_fyle_payments:
          advancedSettingsForm.get('paymentSync')?.value &&
          advancedSettingsForm.get('paymentSync')?.value === QBOPaymentSyncDirection.QBO_TO_FYLE
            ? true
            : false,
        ...
        je_single_credit_line: !brandingFeatureConfig.featureFlags.advancedSettings.singleCreditLineJE
          ? true
          : advancedSettingsForm.get('singleCreditLineJE')?.value,
      ...
      general_mappings: {
        bill_payment_account: advancedSettingsForm.get('billPaymentAccount')?.value
          ? advancedSettingsForm.get('billPaymentAccount')?.value
