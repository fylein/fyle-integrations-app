...
import {
  QBOExportSettingGet,
  QBOExportSettingPost,
} from 'src/app/core/models/qbo/qbo-configuration/qbo-export-setting.model';
...
export class QboExportSettingsService extends ExportSettingsService {
  ...
  static getMandatoryField(
    form: FormGroup,
    employeeMappingValue: EmployeeFieldMapping,
    ...
    switch (controllerName) {
      case 'bankAccount':
        return (
          ...
          form.controls.reimbursableExportType.value &&
          form.controls.reimbursableExportType.value !== QBOReimbursableExpensesObject.EXPENSE
        );
      case 'accountsPayable':
        return (
          form.controls.reimbursableExportType.value === QBOReimbursableExpensesObject.BILL ||
          (form.controls.reimbursableExportType.value === QBOReimbursableExpensesObject.JOURNAL_ENTRY &&
            form.controls.employeeMapping.value === EmployeeFieldMapping.VENDOR) ||
          form.controls.creditCardExportType.value === QBOCorporateCreditCardExpensesObject.BILL
        );
      case 'defaultCCCAccount':
        return (
          form.controls.creditCardExportType.value &&
          form.controls.creditCardExportType.value !== QBOCorporateCreditCardExpensesObject.BILL &&
          form.controls.creditCardExportType.value !== QBOCorporateCreditCardExpensesObject.DEBIT_CARD_EXPENSE
        );
      ...
      case 'qboExpenseAccount':
        return (
          form.controls.reimbursableExportType.value === QBOReimbursableExpensesObject.EXPENSE ||
          form.controls.creditCardExportType.value === QBOCorporateCreditCardExpensesObject.EXPENSE
        );
  ...
  mapAPIResponseToFormGroup(
    ...
    return new FormGroup({
      ...
      reimbursableExpense: new FormControl(
        exportSettings?.workspace_general_settings?.reimbursable_expenses_object ? true : false,
      ),
      ...
      creditCardExpense: new FormControl(
        exportSettings?.workspace_general_settings?.corporate_credit_card_expenses_object ? true : false,
      ),
      ...
      bankAccount: new FormControl(
        exportSettings?.general_mappings?.bank_account?.id ? exportSettings.general_mappings.bank_account : null,
      ),
      defaultCCCAccount: new FormControl(
      ...
      qboExpenseAccount: new FormControl(
        exportSettings?.general_mappings?.qbo_expense_account?.id
          ? exportSettings.general_mappings.qbo_expense_account
          : null,
      ),
      defaultDebitCardAccount: new FormControl(
        exportSettings?.general_mappings?.default_debit_card_account?.id
          ? exportSettings.general_mappings.default_debit_card_account
          : null,
      ),
      nameInJournalEntry: new FormControl(
        exportSettings?.workspace_general_settings?.name_in_journal_entry
          ? exportSettings.workspace_general_settings?.name_in_journal_entry
    ...
    const exportSettingPayload: QBOExportSettingPost = {
      ...
      general_mappings: {
        ...
        default_ccc_account: exportSettingsForm.get('defaultCCCAccount')?.value
          ? exportSettingsForm.get('defaultCCCAccount')?.value
          : emptyDestinationAttribute,
        ...
        qbo_expense_account: exportSettingsForm.get('qboExpenseAccount')?.value
          ? exportSettingsForm.get('qboExpenseAccount')?.value
          : emptyDestinationAttribute,
        default_debit_card_account: exportSettingsForm.get('defaultDebitCardAccount')?.value
          ? exportSettingsForm.get('defaultDebitCardAccount')?.value
          : emptyDestinationAttribute,
      },
    };
  ...
  static createEmployeeSettingsForm(
  ...
  setupDynamicValidators(
    form: FormGroup,
    employeeMappingControl: AbstractControl | null,
    values: ExportModuleRule,
    ...
    Object.entries(values.requiredValue).forEach(([key, value]) => {
      if (key === selectedValue) {
        value.forEach((formController: string) => {
          ...
          const isFieldMandatory = QboExportSettingsService.getMandatoryField(
            form,
            employeeMappingControl?.value,
            formController,
          );
          if (isFieldMandatory) {
