...
export class CommonResourcesService {
  ...
  getCachedDisplayName({
    ...
    sourceType: 'FYLE' | 'ACCOUNTING';
  }): string | undefined {
    const cachedDimensionDetail = this.dimensionDetailCache.find(
      (dimensionDetail) =>
        dimensionDetail.attribute_type === attributeType && dimensionDetail.source_type === sourceType,
    );
  ...
  getDimensionDetails({
    ...
    const observable = this.apiService
      .get(`/workspaces/${this.workspaceService.getWorkspaceId()}/common_resources/dimension_details/`, params)
      .pipe(share());

    observable.subscribe((paginatedDimensionDetails) => {
