...
export class AdvancedSettingsService {
  ...
  static getMemoOptions(
    exportSettings: IntacctConfiguration | ExportSettingGet | NetSuiteExportSettingGet | QBOExportSettingGet,
    appName: string,
  ): string[] {
    ...
    if (
      ...
      !brandingFeatureConfig.featureFlags.advancedSettings.excludeCardNumberAndEmployeeNameInMemo
    ) {
   ...
   * @returns string[]
   */
  static getTopLevelMemoOptions(
    reimbursableGroupedBy?: ExpenseGroupingFieldOption,
    cccGroupedBy?: ExpenseGroupingFieldOption,
  ): string[] {
    ...
    if (reimbursableGroupedBy === cccGroupedBy) {
      ...
      finalOption =
        reimbursableGroupedBy === ExpenseGroupingFieldOption.EXPENSE_ID ? 'expense_number' : 'report_number';
    } else {
  ...
  getExportFrequency(
    isRealTimeExportEnabled: boolean | undefined,
    isOnboarding: boolean,
    autoImportExportEnabled: boolean | undefined,
