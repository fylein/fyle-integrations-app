...
export class ImportSettingsService {
    ...
    return [
      {
        ...
        display_name: this.translocoService.translate('services.importSettings.createCustomField'),
  ...
  getImportCodeField(
    importCodeFields: string[],
    destinationField: string,
    importCodeFieldCodeConfig: ImportCodeFieldConfigType,
  ): boolean | null {
  ...
  constructFormArray(
    importSettingsMappingSettings: ImportSettingMappingRow[],
    accountingAppFields: IntegrationField[],
    importCodeFieldCodeConfig?: ImportCodeFieldConfigType,
    isDestinationFixedImport: boolean = true,
    importCodeFields: string[] | [] = [],
  ): FormGroup[] {
    ...
    accountingAppFields.forEach((accountingAppField) => {
      ...
      if (mappingSetting) {
        fieldData.import_code = importCodeFieldCodeConfig
          ? this.getImportCodeField(importCodeFields, accountingAppField.attribute_type, importCodeFieldCodeConfig)
          : null;
  ...
  constructMappingSettingPayload(expenseFieldArray: ImportSettingMappingRow[]): ImportSettingMappingRow[] {
    const filteredExpenseFieldArray = expenseFieldArray.filter(
      (field: ImportSettingMappingRow) => field.destination_field && field.source_field,
    );
