...
import {
  XeroImportSettingGet,
  XeroImportSettingPost,
} from 'src/app/core/models/xero/xero-configuration/xero-import-settings.model';
...
export class XeroImportSettingsService extends ImportSettingsService {
  ...
  mapAPIResponseToFormGroup(
    importSettings: XeroImportSettingGet | null,
    ...
    const expenseFieldsArray = importSettings?.mapping_settings
      ? additionalOption.concat(this.constructFormArray(importSettings.mapping_settings, xeroFields))
      : [];
    ...
    return new FormGroup({
      ...
      chartOfAccountTypes: new FormControl(
        importSettings?.workspace_general_settings.charts_of_accounts
          ? importSettings.workspace_general_settings.charts_of_accounts.map(
      ...
      importSuppliersAsMerchants: new FormControl(
        importSettings?.workspace_general_settings.import_suppliers_as_merchants ?? false,
      ),
      defaultTaxCode: new FormControl(
        importSettings?.general_mappings?.default_tax_code?.id
          ? findObjectByDestinationId(destinationAttribute, importSettings.general_mappings.default_tax_code.id)
          : null,
      ),
  ...
  constructPayload(importSettingsForm: FormGroup, isCloneSettings: boolean = false): XeroImportSettingPost {
    ...
    const expenseFieldArray = importSettingsForm
      .getRawValue()
      .expenseFields.filter((data: any) => data.destination_field !== XeroFyleField.CUSTOMER);
    ...
    if (importSettingsForm.get('defaultTaxCode')?.value) {
      if (isCloneSettings) {
        ...
        defaultTaxCode = this.exportSettingsService.formatGeneralMappingPayload(
    ...
    const importSettingPayload: XeroImportSettingPost = {
      workspace_general_settings: {
        ...
        import_customers: importSettingsForm.get('importCustomers')?.value
          ? importSettingsForm.get('importCustomers')?.value
          : false,
      },
      general_mappings: {
