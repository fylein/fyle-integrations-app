...
import {
  XeroExportSettingGet,
  XeroExportSettingPost,
} from 'src/app/core/models/xero/xero-configuration/xero-export-settings.model';
...
export class XeroExportSettingsService {
  ...
  getAutoMapEmployeeOptions(): SelectFormOption[] {
    return [
      ...
      {
        label: this.translocoService.translate('services.xeroExportSettings.employeeNameToContactName', {
          brandName: brandingConfig.brandName,
        }),
        value: AutoMapEmployeeOptions.NAME,
      },
      {
        label: this.translocoService.translate('services.xeroExportSettings.employeeEmailToContactEmail', {
          brandName: brandingConfig.brandName,
        }),
  ...
  mapAPIResponseToFormGroup(
    exportSettings: XeroExportSettingGet | null,
    ...
    return new FormGroup({
      ...
      reimbursableExportType: new FormControl(
        exportSettings?.workspace_general_settings?.reimbursable_expenses_object
          ? exportSettings?.workspace_general_settings?.reimbursable_expenses_object
          : XeroReimbursableExpensesObject.PURCHASE_BILL,
      ),
      ...
      creditCardExportType: new FormControl(
        exportSettings?.workspace_general_settings?.corporate_credit_card_expenses_object
          ? exportSettings?.workspace_general_settings?.corporate_credit_card_expenses_object
          : XeroCorporateCreditCardExpensesObject.BANK_TRANSACTION,
      ),
      ...
      bankAccount: new FormControl(
        exportSettings?.general_mappings?.bank_account?.id
          ? findObjectByDestinationId(destinationAttribute, exportSettings.general_mappings.bank_account.id)
          : null,
      ),
  ...
  constructPayload(exportSettingsForm: FormGroup, isCloneSettings: boolean = false): XeroExportSettingPost {
    ...
    if (exportSettingsForm.get('bankAccount')?.value) {
      if (isCloneSettings) {
        ...
        bankAccount = this.exportSettingsService.formatGeneralMappingPayload(
    ...
    const exportSettingPayload: XeroExportSettingPost = {
      ...
      workspace_general_settings: {
        reimbursable_expenses_object: exportSettingsForm.get('reimbursableExpense')?.value
          ? XeroReimbursableExpensesObject.PURCHASE_BILL
          : null,
        corporate_credit_card_expenses_object: exportSettingsForm.get('creditCardExpense')?.value
          ? XeroCorporateCreditCardExpensesObject.BANK_TRANSACTION
          : null,
