...
import {
  EmployeeMapping,
  EmployeeMappingPost,
  EmployeeMappingsResponse,
} from 'src/app/core/models/intacct/db/employee-mapping.model';
...
export class SiMappingsService {
  ...
  getPaginatedDestinationAttributes(
    ...
    value?: string,
  ): Observable<PaginatedintacctDestinationAttribute> {
    const workspaceId = this.workspaceService.getWorkspaceId();
  ...
  getSageIntacctDestinationAttributes(
    attributeTypes: string | string[],
    accountType?: string,
    active?: boolean,
  ): Observable<IntacctDestinationAttribute[]> {
    const workspaceId = this.workspaceService.getWorkspaceId();
  ...
  getGroupedDestinationAttributes(attributeTypes: string[]): Observable<GroupedDestinationAttribute> {
    return from(
      this.getSageIntacctDestinationAttributes(attributeTypes)
        .toPromise()
        .then((response: IntacctDestinationAttribute[] | undefined) => {
          return response?.reduce(
            (groupedAttributes: GroupedDestinationAttribute | any, attribute: IntacctDestinationAttribute) => {
  ...
  getCategoryMappings(
  ...
  ): Observable<CategoryMappingsResponse> {
    const workspaceId = this.workspaceService.getWorkspaceId();
    ...
    return this.apiService.get(`/workspaces/${workspaceId}/mappings/category_attributes/`, params);
  }
  ...
  getEmployeeMappings(
    ...
    mappingState: MappingState,
    alphabetsFilter: string,
  ): Observable<EmployeeMappingsResponse> {
    const workspaceId = this.workspaceService.getWorkspaceId();
    const isMapped: boolean = mappingState === 'UNMAPPED' ? false : true;
    const params: {
      limit: number;
    ...
    return this.apiService.get(`/workspaces/${workspaceId}/mappings/employee_attributes/`, params);
  }

  postEmployeeMappings(employeeMapping: EmployeeMappingPost): Observable<EmployeeMapping> {
    return this.apiService.post(
      `/workspaces/${this.workspaceService.getWorkspaceId()}/mappings/employee/`,
      employeeMapping,
    );
  }
  ...
  postMappingSettings(mappingSettings: MappingSetting[]): Observable<MappingSetting[]> {
    return this.apiService.post(
      `/workspaces/${this.workspaceService.getWorkspaceId()}/mappings/settings/`,
      mappingSettings,
    );
  }
  ...
  triggerAutoMapEmployees() {
    return this.apiService.post(
      `/workspaces/${this.workspaceService.getWorkspaceId()}/mappings/auto_map_employees/trigger/`,
      {},
    );
  }
  ...
  getMappings(
    ...
    alphabetsFilter: string,
  ): Observable<ExtendedExpenseAttributeResponse> {
    const isMapped: boolean = mappingState === 'UNMAPPED' ? false : true;
    const params: any = {
      limit,
      offset,
    ...
    return this.apiService.get(
      `/workspaces/${this.workspaceService.getWorkspaceId()}/mappings/expense_attributes/`,
      params,
    );
  }
