  ...
  static constructPayload(exportSettingsForm: FormGroup): ExportSettingPost {
    ...
    const getExpenseGroupFields = (exportGroupValue: ExpenseGroupingFieldOption) => {
      ...
      if (exportGroupValue === ExpenseGroupingFieldOption.EXPENSE_ID) {
        ...
        exportGroupValue === ExpenseGroupingFieldOption.CLAIM_NUMBER ||
        exportGroupValue === ExpenseGroupingFieldOption.REPORT_ID
      ) {
    ...
    let employeeFieldMapping = exportSettingsForm.get('employeeFieldMapping')?.value
      ? exportSettingsForm.get('employeeFieldMapping')?.value.toUpperCase()
      : 'VENDOR';
    if (
      exportSettingsForm.get('creditCardExportType')?.value === IntacctCorporateCreditCardExpensesObject.EXPENSE_REPORT
    ) {
    ...
    const exportSettingPayload: ExportSettingPost = {
      expense_group_settings: {
        ...
        ccc_export_date_type:
          getValueOrDefault(exportSettingsForm.get('cccExportDate')) === 'Spend date'
            ? 'spent_at'
            : getValueOrDefault(exportSettingsForm.get('cccExportDate')),
      ...
      configurations: {
        ...
        use_merchant_in_journal_line: exportSettingsForm.get('useMerchantInJournalLine')?.value
          ? exportSettingsForm.get('useMerchantInJournalLine')?.value
          : false,
      },
      general_mappings: {
        default_gl_account: exportSettingsForm.get('glAccount')?.value?.value
          ? {
              id: exportSettingsForm.get('glAccount')?.value.destination_id,
              name: exportSettingsForm.get('glAccount')?.value.value,
            }
          : emptyDestinationAttribute,
        default_credit_card: exportSettingsForm.get('creditCard')?.value?.value
          ? {
              id: exportSettingsForm.get('creditCard')?.value.destination_id,
              name: exportSettingsForm.get('creditCard')?.value.value,
            }
          : emptyDestinationAttribute,
        default_charge_card: exportSettingsForm.get('chargeCard')?.value?.value
          ? {
              id: exportSettingsForm.get('chargeCard')?.value.destination_id,
              name: exportSettingsForm.get('chargeCard')?.value.value,
            }
          : emptyDestinationAttribute,
        default_ccc_expense_payment_type: exportSettingsForm.get('cccExpensePaymentType')?.value?.value
          ? {
              id: exportSettingsForm.get('cccExpensePaymentType')?.value.destination_id,
              name: exportSettingsForm.get('cccExpensePaymentType')?.value.value,
            }
          : emptyDestinationAttribute,
        default_reimbursable_expense_payment_type: exportSettingsForm.get('reimbursableExpensePaymentType')?.value
          ?.value
          ? {
              id: exportSettingsForm.get('reimbursableExpensePaymentType')?.value.destination_id,
              name: exportSettingsForm.get('reimbursableExpensePaymentType')?.value.value,
            }
          : emptyDestinationAttribute,
        default_ccc_vendor: exportSettingsForm.get('creditCardVendor')?.value?.value
          ? {
              id: exportSettingsForm.get('creditCardVendor')?.value.destination_id,
              name: exportSettingsForm.get('creditCardVendor')?.value.value,
            }
          : emptyDestinationAttribute,
      },
    };
