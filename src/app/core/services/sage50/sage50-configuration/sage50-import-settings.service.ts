...
import {
  ...
  Sage50ImportSettingsGet,
  Sage50ImportSettingsPost,
  Sage50MappingSettingRow,
...
export class Sage50ImportSettingsService {
    ...
    return new FormGroup<Sage50ImportSettingsForm>(
      {
        ...
        PHASE: new FormGroup({
          ...
          destinationField: new FormControl(Sage50ImportableField.PHASE as Sage50ImportableField, {
            nonNullable: true,
          }),
        ...
        COST_CODE: new FormGroup({
          ...
          destinationField: new FormControl(Sage50ImportableField.COST_CODE as Sage50ImportableField, {
            nonNullable: true,
          }),
  ...
  constructPayloadAndPost(importSettingsForm: FormGroup<Sage50ImportSettingsForm>): Observable<void> {
    const importCodeFields = Object.values(Sage50ImportableField).filter(
      (field) => importSettingsForm.get(field)?.get('importCode')?.value,
    );
    ...
    return this.putSage50ImportSettings({
      import_settings: {
        ...
        chart_of_accounts: importSettingsForm.get('ACCOUNT')?.get('accountTypes')?.value ?? [
          Sage50ImportableCOAType.EXPENSES,
        ],
      },
