...
type DependencyChecker = (form: AbstractControl, showToggles?: boolean) => boolean;
...
export const FIELD_DEPENDENCIES = new Map<keyof Sage50ExportSettingsForm, DependencyChecker>([
  ...
  [
    'reimbursableDefaultCreditLineAccount',
    (form) => form.get('reimbursableExportType')?.value === Sage50ReimbursableExportType.GENERAL_JOURNAL_ENTRY,
  ],
  [
    'reimbursableDefaultAccountPayableAccount',
    (form) => form.get('reimbursableExportType')?.value === Sage50ReimbursableExportType.PURCHASES_RECEIVE_INVENTORY,
  ],
  ...
  [
    ...
    (form) => !!form.get('reimbursableExportType')?.value && !!form.get('reimbursableExportGroup')?.value,
  ],
  ['cccExpenses', (_, showToggles) => !!showToggles], // Show toggles as necessary
  ['cccExportType', (form) => !!form.get('cccExpenses')?.value],
  [
    'cccDefaultCreditLineAccount',
    (form) => form.get('cccExportType')?.value === Sage50CCCExportType.GENERAL_JOURNAL_ENTRY,
  ],
  [
    'cccDefaultAccountPayableAccount',
    (form) => form.get('cccExportType')?.value === Sage50CCCExportType.PURCHASES_RECEIVE_INVENTORY,
  ],
  ['defaultCashAccount', (form) => form.get('cccExportType')?.value === Sage50CCCExportType.PAYMENTS_JOURNAL],
  [
    'defaultVendor',
    (form) =>
      [Sage50CCCExportType.PURCHASES_RECEIVE_INVENTORY, Sage50CCCExportType.PAYMENTS_JOURNAL].includes(
        form.get('cccExportType')?.value!,
      ),
  ],
  ['defaultPaymentMethod', (form) => form.get('cccExportType')?.value === Sage50CCCExportType.PAYMENTS_JOURNAL],
  ['cccExpenseState', (form) => !!form.get('cccExportType')?.value],
  ['cccExportGroup', (form) => !!form.get('cccExportType')?.value],
  ['cccExportDate', (form) => !!form.get('cccExportType')?.value && !!form.get('cccExportGroup')?.value],
]);
...
export class Sage50ExportSettingsService extends ExportSettingsService {
  ...
  getExportSettings(): Observable<Sage50ExportSettingsGet | null> {
    return this.apiService
      .get(`/${this.workspaceService.getWorkspaceId()}/settings/export_settings/`, {})
  ...
  mapApiResponseToFormGroup(
    ...
    const { reimbursableExpenses, cccExpenses } = this.getEnabledExportTypes(
      apiResponse,
    ...
    return new FormGroup<Sage50ExportSettingsForm>(
      {
        ...
        reimbursableDefaultAccountPayableAccount: new FormControl(
          apiResponse?.reimbursable_default_account_payable_account ?? null,
        ),
        reimbursableDefaultCreditLineAccount: new FormControl(
          apiResponse?.reimbursable_default_credit_line_account ?? null,
        ),
