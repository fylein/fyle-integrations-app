...
import {
  TravelperkAdvancedSettingGet,
  TravelperkAdvancedSettingPost,
  TravelperkAdvancedSettingArray,
  TravelperkAdvancedSettingFormArray,
  TravelperkCategoryMapping,
} from '../../../models/travelperk/travelperk-configuration/travelperk-advanced-settings.model';
...
export class TravelperkAdvancedSettingsService {
  ...
  constructFormArray(
    arrayData: TravelperkAdvancedSettingGet,
    source_fields: TravelperkDestinationAttribuite[],
  ): FormGroup[] {
    ...
    Object.entries(arrayData.category_mappings).forEach((value, index) => {
      const findObjectByDestinationId = (array: TravelperkDestinationAttribuite[], id: string) =>
        array?.find((item) => item.source_id === id) || null;
      const data: TravelperkAdvancedSettingArray = {
        ...
        source_name: arrayData.category_mappings[categoryMapping[index]]
          ? findObjectByDestinationId(source_fields, arrayData.category_mappings[categoryMapping[index]].id)
          : null,
      };
  ...
  mapAPIResponseToFormGroup(
    ...
    sourceOptions: TravelperkDestinationAttribuite[],
  ): FormGroup {
    ...
    return new FormGroup({
      ...
      descriptionStructure: new FormControl(
        advancedSettings?.description_structure ? advancedSettings?.description_structure : defaultMemoOptions,
      ),
      defaultEmployee: new FormControl(
        advancedSettings?.default_employee_name ? advancedSettings?.default_employee_name : null,
      ),
      defaultEmployeeId: new FormControl(
        advancedSettings?.default_employee_id ? advancedSettings?.default_employee_id : null,
      ),
      defaultCategory: new FormControl(
        advancedSettings?.default_category_name
          ? findObjectByDestinationId(sourceOptions, advancedSettings.default_category_id)
          : null,
        Validators.required,
      ),
      invoiceLineitemStructure: new FormControl(
        advancedSettings?.invoice_lineitem_structure ? advancedSettings.invoice_lineitem_structure : null,
        Validators.required,
      ),
  ...
  createAdvancedSettingPayload(advancedSettingsForm: FormGroup): TravelperkAdvancedSettingPost {
    return {
      default_employee_name: advancedSettingsForm.get('defaultEmployee')?.value
        ? advancedSettingsForm.get('defaultEmployee')?.value
        : null,
      default_employee_id: advancedSettingsForm.get('defaultEmployeeId')?.value
        ? advancedSettingsForm.get('defaultEmployeeId')?.value
        : null,
      default_category_name: advancedSettingsForm.get('defaultCategory')?.value
        ? advancedSettingsForm.get('defaultCategory')?.value.value
        : null,
      default_category_id: advancedSettingsForm.get('defaultCategory')?.value
        ? advancedSettingsForm.get('defaultCategory')?.value.source_id
        : null,
      description_structure: advancedSettingsForm.get('descriptionStructure')?.value
        ? advancedSettingsForm.get('descriptionStructure')?.value
        : null,
      invoice_lineitem_structure: advancedSettingsForm.get('invoiceLineitemStructure')?.value
        ? advancedSettingsForm.get('invoiceLineitemStructure')?.value
        : null,
