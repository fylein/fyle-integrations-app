...
export class QbdDirectImportSettingsService extends ImportSettingsService {
  ...
  getChartOfAccountTypesList(): string[] {
    const typeList = [
      this.translocoService.translate('services.qbdDirectImportSettings.otherExpense'),
      this.translocoService.translate('services.qbdDirectImportSettings.costOfGoodsSold'),
      this.translocoService.translate('services.qbdDirectImportSettings.fixedAsset'),
      this.translocoService.translate('services.qbdDirectImportSettings.otherAsset'),
      this.translocoService.translate('services.qbdDirectImportSettings.otherCurrentAsset'),
      this.translocoService.translate('services.qbdDirectImportSettings.longTermLiability'),
      this.translocoService.translate('services.qbdDirectImportSettings.otherCurrentLiability'),
      this.translocoService.translate('services.qbdDirectImportSettings.income'),
      this.translocoService.translate('services.qbdDirectImportSettings.otherIncome'),
      this.translocoService.translate('services.qbdDirectImportSettings.equity'),
  ...
  mapAPIResponseToFormGroup(
    ...
    const expenseFieldsArray = importSettings?.mapping_settings
      ? this.constructFormArray(importSettings?.mapping_settings, QbdDirectFields, QbdDirectImportCodeFieldCodeConfig)
      : [];
    return new FormGroup({
      ...
      chartOfAccountTypes: new FormControl(
        importSettings?.import_settings?.chart_of_accounts
          ? importSettings.import_settings.chart_of_accounts.map((item) => item.replace(/([a-z])([A-Z])/g, '$1 $2'))
          : [this.translocoService.translate('services.qbdDirectImportSettings.expense')],
      ),
      ...
      importCategoryCode: new FormControl(
        this.getImportCodeField(importCode, 'ACCOUNT', QbdDirectImportCodeFieldCodeConfig),
      ),
  ...
  constructPayload(importSettingsForm: FormGroup): QbdDirectImportSettingPost {
    ...
    const coaArray = importSettingsForm
      .get('chartOfAccountTypes')
      ?.value.map((item: string) => item.replace(/\s+/g, ''));
