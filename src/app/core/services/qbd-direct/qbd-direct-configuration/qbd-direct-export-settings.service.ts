...
import {
  QbdDirectExportSettingGet,
  QbdDirectExportSettingsPost,
} from 'src/app/core/models/qbd-direct/qbd-direct-configuration/qbd-direct-export-settings.model';
...
import {
  ...
  QbdDirectExpenseGroupBy,
  QbdDirectReimbursableExpensesObject,
  QbdDirectReimbursableExportDateType,
  QBDExpenseGroupedBy,
  QBDExportDateType,
...
export class QbdDirectExportSettingsService extends ExportSettingsService {
  ...
  setReimbursableExpenseGroupingDateOptions(
    reimbursableExportGroup: QbdDirectExpenseGroupBy,
    ...
    switch (controllerName) {
      ...
      case 'employeeMapping':
        return (
          ...
          form.get('creditCardExportType')?.value &&
    ...
    const exportModuleRule: ExportModuleRule[] = [
      {
        ...
        requiredValue: {
          ...
          [QbdDirectReimbursableExpensesObject.JOURNAL_ENTRY]: [
            'defaultReimbursableAccountsPayableAccountName',
            'employeeMapping',
          ],
        },
      },
      {
        ...
        requiredValue: {
          ...
          [QBDCorporateCreditCardExpensesObject.JOURNAL_ENTRY]: [
            'defaultCCCAccountsPayableAccountName',
            'nameInJE',
            'employeeMapping',
          ],
        },
  ...
  mapAPIResponseToFormGroup(
    ...
    return new FormGroup({
      ...
      creditCardExportType: new FormControl(
        exportSettings?.credit_card_expense_export_type ? exportSettings?.credit_card_expense_export_type : null,
      ),
      creditCardExportGroup: new FormControl(
        exportSettings?.credit_card_expense_grouped_by
          ? exportSettings?.credit_card_expense_grouped_by
          : this.expenseGroupingFieldOptions()[1].value,
      ),
      creditCardExportDate: new FormControl(
        exportSettings?.credit_card_expense_date
          ? exportSettings?.credit_card_expense_date
          : this.expenseGroupingFieldOptions()[0].value,
      ),
      ...
      creditCardExpenseState: new FormControl(
        exportSettings?.credit_card_expense_state ? exportSettings?.credit_card_expense_state : null,
      ),
      employeeMapping: new FormControl(
        exportSettings?.employee_field_mapping ? exportSettings?.employee_field_mapping : EmployeeFieldMapping.VENDOR,
      ),
      CCCEmployeeMapping: new FormControl(
        exportSettings?.employee_field_mapping ? exportSettings?.employee_field_mapping : EmployeeFieldMapping.VENDOR,
      ),
      nameInJE: new FormControl(
        exportSettings?.name_in_journal_entry ? exportSettings?.name_in_journal_entry : this.nameInJEOptions()[0].value,
      ),
      defaultCreditCardAccountName: new FormControl(
        exportSettings?.default_credit_card_account_id
          ? findObjectByDestinationId(destinationAccounts, exportSettings.default_credit_card_account_id)
          : null,
      ),
      defaultReimbursableAccountsPayableAccountName: new FormControl(
        exportSettings?.default_reimbursable_accounts_payable_account_id
      ...
      defaultCCCAccountsPayableAccountName: new FormControl(
        exportSettings?.default_ccc_accounts_payable_account_id
          ? findObjectByDestinationId(destinationAccounts, exportSettings.default_ccc_accounts_payable_account_id)
          : null,
      ),
      cccPurchasedFromField: new FormControl(
        exportSettings?.ccc_purchased_from_field ? exportSettings?.ccc_purchased_from_field : null,
      ),
  ...
  static constructPayload(exportSettingsForm: FormGroup): QbdDirectExportSettingsPost {
    ...
    const exportSettingPayload: QbdDirectExportSettingsPost = {
      reimbursable_expense_export_type: exportSettingsForm.get('reimbursableExportType')?.value
      ...
      reimbursable_expense_date: exportSettingsForm.get('reimbursableExportDate')?.value
        ...
        : null,
      reimbursable_expense_grouped_by:
        exportSettingsForm.get('reimbursableExpense')?.value && exportSettingsForm.get('reimbursableExportGroup')?.value
          ? exportSettingsForm.get('reimbursableExportGroup')?.value
          : null,
      credit_card_expense_export_type: exportSettingsForm.get('creditCardExportType')?.value
        ? exportSettingsForm.get('creditCardExportType')?.value
        : null,
      credit_card_expense_state: exportSettingsForm.get('creditCardExpenseState')?.value
        ? exportSettingsForm.get('creditCardExpenseState')?.value
        : null,
      credit_card_expense_grouped_by:
        exportSettingsForm.get('creditCardExpense')?.value && exportSettingsForm.get('creditCardExportGroup')?.value
          ? exportSettingsForm.get('creditCardExportGroup')?.value
          : null,
      credit_card_expense_date:
        exportSettingsForm.get('creditCardExpense')?.value && exportSettingsForm.get('creditCardExportDate')?.value
          ? exportSettingsForm.get('creditCardExportDate')?.value
          : null,
      employee_field_mapping: employeeFieldMapping,
      name_in_journal_entry: nameInJournalEntry,
      default_credit_card_account_name: exportSettingsForm.get('defaultCreditCardAccountName')?.value
        ? exportSettingsForm.get('defaultCreditCardAccountName')?.value.value
        : null,
      default_credit_card_account_id: exportSettingsForm.get('defaultCreditCardAccountName')?.value
        ? exportSettingsForm.get('defaultCreditCardAccountName')?.value.destination_id
        : null,
      default_reimbursable_accounts_payable_account_name: exportSettingsForm.get(
        'defaultReimbursableAccountsPayableAccountName',
      )?.value
        ? exportSettingsForm.get('defaultReimbursableAccountsPayableAccountName')?.value.value
        : null,
      default_reimbursable_accounts_payable_account_id: exportSettingsForm.get(
        'defaultReimbursableAccountsPayableAccountName',
      )?.value
        ? exportSettingsForm.get('defaultReimbursableAccountsPayableAccountName')?.value.destination_id
        : null,
      default_ccc_accounts_payable_account_name: exportSettingsForm.get('defaultCCCAccountsPayableAccountName')?.value
        ? exportSettingsForm.get('defaultCCCAccountsPayableAccountName')?.value.value
        : null,
      default_ccc_accounts_payable_account_id: exportSettingsForm.get('defaultCCCAccountsPayableAccountName')?.value
        ? exportSettingsForm.get('defaultCCCAccountsPayableAccountName')?.value.destination_id
        : null,
      ccc_purchased_from_field: exportSettingsForm.get('cccPurchasedFromField')?.value
        ? exportSettingsForm.get('cccPurchasedFromField')?.value
        : null,
    };
