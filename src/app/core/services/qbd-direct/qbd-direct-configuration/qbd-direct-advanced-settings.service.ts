...
import {
  QbdDirectAdvancedSettingsGet,
  QbdDirectAdvancedSettingsPost,
} from 'src/app/core/models/qbd-direct/qbd-direct-configuration/qbd-direct-advanced-settings.model';
...
export class QbdDirectAdvancedSettingsService extends AdvancedSettingsService {
  ...
  mapAPIResponseToFormGroup(
    advancedSettings: QbdDirectAdvancedSettingsGet | null,
    isSkipExportEnabled: boolean,
    isOnboarding: boolean,
  ): FormGroup {
    return new FormGroup({
      expenseMemoStructure: new FormControl(
        advancedSettings?.line_level_memo_structure && advancedSettings?.line_level_memo_structure.length > 0
          ? QbdDirectAdvancedSettingsService.formatMemoStructure(
              QbdDirectAdvancedSettingsService.defaultMemoFields(),
              advancedSettings?.line_level_memo_structure,
            )
          : QbdDirectAdvancedSettingsService.defaultMemoFields(),
        Validators.required,
      ),
      topMemoStructure: new FormControl(
        advancedSettings?.top_level_memo_structure && advancedSettings?.top_level_memo_structure.length > 0
          ? advancedSettings?.top_level_memo_structure
          : QbdDirectAdvancedSettingsService.defaultTopMemoOptions(),
        Validators.required,
      ),
      ...
      autoCreateReimbursableEnitity: new FormControl(
        advancedSettings?.auto_create_reimbursable_entity ? advancedSettings?.auto_create_reimbursable_entity : false,
      ),
      autoCreateMerchantsAsVendors: new FormControl(
        advancedSettings?.auto_create_merchant_as_vendor ? advancedSettings?.auto_create_merchant_as_vendor : false,
      ),
  ...
  static constructPayload(advancedSettingForm: FormGroup, adminEmails: EmailOption[]): QbdDirectAdvancedSettingsPost {
    ...
    const memo = advancedSettingForm.get('expenseMemoStructure')?.value
      ? advancedSettingForm.get('expenseMemoStructure')?.value
      : [];

    const advancedSettingPayload: QbdDirectAdvancedSettingsPost = {
      line_level_memo_structure: advancedSettingForm.get('expenseMemoStructure')?.value
        ? QbdDirectAdvancedSettingsService.formatMemoStructure(
            QbdDirectAdvancedSettingsService.defaultMemoFields(),
            memo,
          )
        : [],
      top_level_memo_structure: advancedSettingForm.get('topMemoStructure')?.value
        ? advancedSettingForm.get('topMemoStructure')?.value
        : null,
      schedule_is_enabled: advancedSettingForm.get('exportSchedule')?.value
        ? advancedSettingForm.get('exportSchedule')?.value
        : false,
      emails_selected: advancedSettingForm.get('exportSchedule')?.value ? selectedEmailsEmails : [],
      interval_hours: advancedSettingForm.get('exportSchedule')?.value
        ? advancedSettingForm.get('exportScheduleFrequency')?.value
        : null,
      is_real_time_export_enabled:
        advancedSettingForm.get('exportSchedule')?.value &&
        advancedSettingForm.get('exportScheduleFrequency')?.value === 0
          ? true
          : false,
      auto_create_reimbursable_entity: advancedSettingForm.get('autoCreateReimbursableEnitity')?.value
        ? advancedSettingForm.get('autoCreateReimbursableEnitity')?.value
        : false,
      auto_create_merchant_as_vendor: advancedSettingForm.get('autoCreateMerchantsAsVendors')?.value
        ? advancedSettingForm.get('autoCreateMerchantsAsVendors')?.value
        : false,
  ...
  postQbdAdvancedSettings(
    advancedSettingPayload: QbdDirectAdvancedSettingsPost,
  ): Observable<QbdDirectAdvancedSettingsGet> {
