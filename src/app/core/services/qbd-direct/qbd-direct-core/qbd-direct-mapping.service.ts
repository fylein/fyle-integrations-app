...
export class QbdDirectMappingService {
  ...
  private getDestinationFieldFromSettings(
    workspaceGeneralSetting: QbdDirectExportSettingGet,
  ...
  private fetchDestinationOptions(detailAccountType?: string[]): Observable<QbdDirectDestinationAttribute[]> {
    ...
    if (
      ...
      this.destinationField === AccountingField.ACCOUNT &&
      this.sourceField === FyleField.CATEGORY
    ) {
      return forkJoin([
  ...
  private getCCCAccountOptions(): Observable<QbdDirectDestinationAttribute[]> {
    if (this.cccExpenseObject === QBDCorporateCreditCardExpensesObject.CREDIT_CARD_PURCHASE) {
      ...
      this.cccExpenseObject === QBDCorporateCreditCardExpensesObject.JOURNAL_ENTRY &&
  ...
  initialize(sourceField: string) {
    ...
    return forkJoin([
      ...
      concatMap(([exportSettings, importSettings, mappings]) => {
        ...
        this.chartOfAccounts = importSettings.import_settings.import_account_as_category
          ? importSettings.import_settings.chart_of_accounts.map((item: string) => item.replace(/\s+/g, ''))
