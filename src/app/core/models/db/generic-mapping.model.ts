...
export class MappingClass {
  static constructEmployeeMappingPayload(
    selectedRow: ExtendedGenericMapping,
    event: any,
    ...
    const employeeMapping: EmployeeMappingPost = {
      ...
      destination_vendor: {
        id:
          event.value.attribute_type === FyleField.VENDOR
            ? event.value.id
            : selectedRow.employeemapping?.length && selectedRow.employeemapping[0].destination_vendor
              ? selectedRow.employeemapping[0].destination_vendor?.id
              : null,
      },
      destination_employee: {
        id:
          event.value.attribute_type === FyleField.EMPLOYEE
            ? event.value.id
            : selectedRow.employeemapping?.length && selectedRow.employeemapping[0].destination_employee
              ? selectedRow.employeemapping[0].destination_employee?.id
              : null,
      },
      destination_card_account: {
        id:
          selectedRow.employeemapping?.length && selectedRow.employeemapping[0].destination_card_account
            ? selectedRow.employeemapping[0].destination_card_account?.id
            : null,
      },
  ...
  static constructCategoryMappingPayload(
    selectedRow: ExtendedGenericMapping,
    event: any,
  ...
  static constructGenericMappingPayload(
    selectedRow: ExtendedGenericMapping,
    event: any,
