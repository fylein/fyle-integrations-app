...
export class ImportSettings {
  static constructPayload(
    ...
    existingDependentFieldSettings: DependentFieldSetting | null,
  ): ImportSettingPost {
    ...
    const filteredExpenseFieldArray = expenseFieldArray.filter(
      (field: MappingSetting) => field.destination_field && field.source_field,
    );

    // Then map over the filtered array
    const mappingSettings = filteredExpenseFieldArray
      .filter((field: MappingSetting) => field.source_field !== 'CATEGORY')
      .map((field: MappingSetting) => {
        return {
          source_field: field.source_field.toUpperCase(),
          destination_field: field.destination_field,
          import_to_fyle: field.import_to_fyle,
          is_custom:
            field.source_field.toUpperCase() === 'PROJECT' || field.source_field.toUpperCase() === 'COST_CENTER'
              ? false
              : true,
    ...
    if (brandingFeatureConfig.featureFlags.importSettings.importSettingsV1) {
      isCategoryImportEnabled = importSettingsForm.get('importCategories')?.value
        ? importSettingsForm.get('importCategories')?.value
        : false;
    } else {
      isCategoryImportEnabled =
        filteredExpenseFieldArray.filter(
          (field: MappingSetting) => field.source_field === 'CATEGORY' && field.import_to_fyle,
        ).length > 0
          ? true
          : false;
    }
    ...
    const importCodeFieldArray = importCodeFields
      .filter((field: { import_code: any }) => field.import_code)
      .map((value: { source_field: any }) => {
        return value.source_field;
      });

    const finalimportCodeFieldArray: string[] = importSettingsForm
      .get('importCodeField')
      ?.value.filter((value: string) => value !== 'COST_CODE' && value !== 'COST_TYPE')
      .concat(importCodeFieldArray);

    // Actual Payload
    const importSettingPayload: ImportSettingPost = {
      configurations: {
        import_categories: isCategoryImportEnabled,
        import_tax_codes: importSettingsForm.get('importTaxCodes')?.value
          ? importSettingsForm.get('importTaxCodes')?.value
          : false,
        import_vendors_as_merchants: importSettingsForm.get('importVendorAsMerchant')?.value
          ? importSettingsForm.get('importVendorAsMerchant')?.value
          : false,
