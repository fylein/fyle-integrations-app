...
export class SkipExportModel {
  ...
  static constructSkipExportPayload(valueField: ExpenseFilterPayload, valueOption: any[]): ExpenseFilterPost {
    return {
      ...
      values:
        ...
        valueField.operator === 'isnull' ||
        valueField.condition.field_name === 'report_title' ||
        valueField.condition.type === 'BOOLEAN'
          ? valueField.value
          : valueOption,
      rank: valueField.rank,
  ...
  static setConditionFields(
    response: ExpenseFilterResponse,
    conditionArray: ConditionField[],
    conditionFieldOptions: ConditionField[],
  ) {
    response.results.forEach((element) => {
      const type = conditionFieldOptions?.filter(
