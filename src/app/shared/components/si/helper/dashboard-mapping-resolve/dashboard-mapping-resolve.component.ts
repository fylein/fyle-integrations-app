...
export class DashboardMappingResolveComponent implements OnInit {
    ...
    this.mappingService.postEmployeeMappings(employeeMapping).subscribe(
      () => {
        this.toastService.displayToastMessage(
          ...
          this.translocoService.translate('dashboardMappingResolve.employeeMappingSuccess'),
        );
      },
      () => {
        this.toastService.displayToastMessage(
          ToastSeverity.ERROR,
          this.translocoService.translate('dashboardMappingResolve.somethingWentWrong'),
        );
      },
    );
  }
  ...
  saveCategoryMapping(selectedRow: ExpenseAttribute, event: any) {
    ...
    this.mappingService.postCategoryMappings(categoryMappingsPayload).subscribe(
      () => {
        this.toastService.displayToastMessage(
          ToastSeverity.SUCCESS,
          this.translocoService.translate('dashboardMappingResolve.categoryMappingSuccess'),
        );
      },
      () => {
        this.toastService.displayToastMessage(
  ...
  setupPage() {
    ...
    this.mappingService.getGroupedDestinationAttributes(attributeType).subscribe((response) => {
      ...
      this.fyleEmployeeOptions =
        this.getEmployeeAttributeType() === FyleField.EMPLOYEE ? response.EMPLOYEE : response.VENDOR;
