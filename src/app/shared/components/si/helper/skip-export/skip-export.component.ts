...
export class SkipExportComponent implements OnInit {
  ...
  showInputField2() {
    return (
      this.skipExportForm.value?.condition2?.field_name &&
      this.skipExportForm.value?.condition2?.field_name === 'report_title' &&
      (this.skipExportForm.get('operator2')?.value !== 'is_empty' ||
        this.skipExportForm.get('operator2')?.value !== 'is_not_empty')
    );
  }
  ...
  showChipField2(): boolean {
    return (
      this.skipExportForm.value?.condition2?.field_name !== 'report_title' &&
      (!this.skipExportForm.value?.condition2 ||
        this.skipExportForm.value?.condition2?.type === 'SELECT' ||
        this.skipExportForm.value?.condition2?.type === 'TEXT' ||
        this.skipExportForm.value?.condition2?.type === 'NUMBER') &&
      this.skipExportForm.get('operator2')?.value !== 'is_empty' &&
      this.skipExportForm.get('operator2')?.value !== 'is_not_empty'
    );
  }
  ...
  showDateField2() {
    return (
      this.skipExportForm.value?.condition2?.type === 'DATE' &&
      (this.skipExportForm.get('operator2')?.value !== 'is_empty' ||
        this.skipExportForm.get('operator2')?.value !== 'is_not_empty')
    );
  }
  ...
  saveSkipExportFields() {
    ...
    if (!this.enableSkipExport) {
      ...
      this.advancedSettingsService.postExpenseFilter(payload1).subscribe((skipExport1: SkipExport) => {
        if (valueField.condition2 && valueField.operator2) {
          ...
          this.advancedSettingsService.postExpenseFilter(payload2).subscribe((skipExport2: SkipExport) => {});
        }
      });
    }
  }
  ...
  operatorFieldWatcher() {
    ...
    this.skipExportForm.controls.operator2.valueChanges.subscribe((operatorSelected) => {
      this.valueOption2 = [];
      if (operatorSelected === 'is_empty' || operatorSelected === 'is_not_empty') {
