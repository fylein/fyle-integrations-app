...
export class ConfigurationExportSettingsComponent implements OnInit {
  ...
  private initializeExportSettingsFormWithData(): void {
      this.exportSettingsForm = this.formBuilder.group({
        reimbursableExpense: [(!!this.exportSettings?.configurations?.reimbursable_expenses_object) || null, this.exportSelectionValidator()],
        reimbursableExportType: [this.exportSettings?.configurations?.reimbursable_expenses_object || null],
