...
export class GenericMappingTableComponent implements OnInit {
  ...
  decrementUnmappedCountInNetsuiteEmployeeMapping(mapping: any): void {
    if (this.destinationField === FyleField.EMPLOYEE && mapping[0].destination_employee === null && !this.isDashboardMappingResolve) {
      this.mappingStats.unmapped_attributes_count -= 1;
    } else if (this.destinationField === FyleField.VENDOR && mapping[0].destination_vendor === null && !this.isDashboardMappingResolve) {
      this.mappingStats.unmapped_attributes_count -= 1;
    }
  }
