...
export class ExcludeSelectedOptionsPipe implements PipeTransform {
  transform(options: any[], formArray: FormArray | null, currentIndex: number, fieldName: string): any[] {
    ...
    return options.filter((option) => !selectedValues.includes(option.attribute_type));
  }
}
